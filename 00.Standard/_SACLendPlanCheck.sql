...
--	   AND A.Status = 0
  ...
  ---------------------------
	...
	---------------------------        
    -- EXEC dbo._SCOMCodeDeleteCheck  @CompanySeq,@UserSeq,@LanguageSeq,'_TACLendPlan', '#TACLendPlan','키값'  
...
--guide : '마스터 키 생성' --------------------------
  ...
  -- if @Count >0 
  -- BEGIN
  --   EXEC @MaxSeq = _SCOMCreateSeq @CompanySeq, '_TACLendPlan','키값',@Count --rowcount  
  --        UPDATE #TACLendPlan             
...
--          FROM _TACLendPlan   AS A
--               JOIN   #TACLendPlan    AS B ON  A.'마스터키값'= B.'마스터키값'
