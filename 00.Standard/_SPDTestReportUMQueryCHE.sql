...
IF OBJECT_ID('_SPDTestReportUMQueryCHE') IS NOT NULL 
    DROP PROC _SPDTestReportUMQueryCHE
GO 
...
/*********************************************************************************************************************    
    ...
    작성일 : 2011.04.28 전경만  
********************************************************************************************************************/   
CREATE PROCEDURE _SPDTestReportUMQueryCHE    
...
AS  
    ...
    BEGIN  
        SELECT A.MinorName AS UMSpecName,  
          ...
          LEFT OUTER JOIN _TDAItem AS I WITH(NOLOCK) ON ( I.CompanySeq = A.CompanySeq AND (@ItemSeq = 0 OR @ItemSeq = I.ItemSeq) )
    ...
    BEGIN  
        SELECT A.MinorName AS UMSpecName,  
          ...
          FROM _TDAUMinor AS A  
          LEFT OUTER JOIN _TPDTestReportUMSpec AS B WITH(NOLOCK) ON B.CompanySeq = A.CompanySeq AND B.UMSpec = A.MinorSeq AND (B.TestReportSeq = @TestReportSeq)  
         WHERE A.CompanySeq = @CompanySeq  
           AND A.MajorSeq = 1000723  
         ORDER BY A.MinorSeq  
    END  
