...
IF OBJECT_ID('jongie_SSLDailySalesResult')IS NOT NULL 
    DROP PROC jongie_SSLDailySalesResult
GO 

-- v2013.10.01 

-- 일일판매실적(출력)_jongie by이재천
CREATE PROC jongie_SSLDailySalesResult                
...
AS        
    ...
    SELECT A.InvoiceDate, 
     ...
     WHERE A.CompanySeq = @CompanySeq
       ...
       AND ISNULL(A.DVPlaceSeq,0) <> 0
       
     ORDER BY C.EmpName, A.InvoiceDate, DelvCustName, ItemNo, ItemName
    
    RETURN
GO
exec jongie_SSLDailySalesResult @xmlDocument=N'<ROOT>
  <DataBlock1>
    ...
    <InvoiceDateTo>20131001</InvoiceDateTo>
    <DeptSeq />
    <EmpSeq />
    <CustSeq />
  </DataBlock1>
