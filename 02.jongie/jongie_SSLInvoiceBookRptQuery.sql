...
IF OBJECT_ID('jongie_SSLInvoiceBookRptQuery')IS NOT NULL
    DROP PROC jongie_SSLInvoiceBookRptQuery
GO

-- v2013.10.01 
  
-- 거래명세서품목조회(도서출력)_jongie by이재천  
 CREATE PROC jongie_SSLInvoiceBookRptQuery      
 ...
 AS   
    ...
    CREATE TABLE #TSLInvoiceTemp (WorkingTag NCHAR(1) NULL)    
    EXEC dbo._SCAOpenXmlToTemp @xmlDocument, @xmlFlags, @CompanySeq, @ServiceSeq, 'DataBlock1', '#TSLInvoiceTemp'       
    IF @@ERROR <> 0 RETURN    
 ...
 /***********************************************************************************************************************************************/    
     ...
     SELECT A.InvoiceSeq, A.InvoiceSerl, A.ItemSeq,   
       ...
       FROM #TSLInvoiceTemp AS Z  
       JOIN _TSLInvoiceItem AS A WITH(NOLOCK) ON ( A.InvoiceSeq = Z.InvoiceSeq )  
  ...
  -------------------------------------------------------------------    
     ...
     (  
         ...
         CustNo         NVARCHAR(100),   
         CustOwner      NVARCHAR(100), 
         OutPercent     DECIMAL(19,5), 
         ConsumerPrice  DECIMAL(19,5)
     )  
    ...
    SELECT A.InvoiceSeq,  
           ...
           LEFT(A.InvoiceDate, 4)         AS YY,     --거래명세서일    
           ...
           B.CurAmt                      AS CurAmt,   
           B.CurVAT                      AS CurVAT,  
           CASE WHEN N.UMCustClass = 1008226002 THEN 0 ELSE B.DomAmt END AS DomAmt,             -- 20101224 김준식 추가  
           CASE WHEN N.UMCustClass = 1008226002 THEN 0 ELSE B.DomVAT END AS DomVAT,             -- 20101224 김준식 추가   
           B.Qty                         AS Qty,          
           CASE WHEN N.UMCustClass = 1008226002   
                THEN 0  
           ...
           E.CustNo,   
           E.Owner AS CustOwner,  
           CASE WHEN ISNULL(P.CustPrice,0) = 0   
                THEN 0   
                ELSE (CASE WHEN B.Price IS NOT NULL  
                           THEN B.Price  
                     ...
                     ) / ISNULL(P.CustPrice,0) * 100   
                END AS OutPercent, -- 출고율
           P.CustPrice AS ConsumerPrice -- 소비자가
      FROM #TSLInvoiceTemp AS O  
      JOIN _TSLInvoice              AS A  WITH(NOLOCK) ON ( A.CompanySeq = @CompanySeq AND A.InvoiceSeq = O.InvoiceSeq )   
      ...
      LEFT OUTER JOIN #TTaxUnit     AS Z  WITH(NOLOCK) ON Z.TaxUnit  = ( CASE WHEN ( @pIsAccTax = 4125002 ) AND ( A.InvoiceDate >= @pIsAccTaxDate )    
       ...
       LEFT OUTER JOIN _TDACustClass      AS N WITH(NOLOCK) ON ( N.CompanySeq = @CompanySeq AND N.CustSeq = E.CustSeq AND N.UMajorCustClass = 1008226 )   
       LEFT OUTER JOIN _TSLItemUnitPrice  AS P WITH(NOLOCK) ON ( P.CompanySeq = @CompanySeq AND P.ItemSeq = B.ItemSeq AND CONVERT(NVARCHAR(8),GETDATE(),112) BETWEEN P.SDate AND P.EDate )   
         
     WHERE A.CompanySeq = @CompanySeq    
     ORDER BY O.InvoiceSeq  
      
    SELECT * FROM #TSLInvoice ORDER BY InvoiceSeq  
      
    RETURN  
GO
exec jongie_SSLInvoiceBookRptQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
    <WorkingTag />
    <IDX_NO>2</IDX_NO>
    <DataSeq>1</DataSeq>
    ...
    <InvoiceDate>20130910</InvoiceDate>
    <CustSeq>42524</CustSeq>
    <TABLE_NAME>DataBlock1</TABLE_NAME>
  </DataBlock1>
</ROOT>',@xmlFlags=2,@ServiceSeq=1017055,@WorkingTag=N'',@CompanySeq=1,@LanguageSeq=1,@UserSeq=50322,@PgmSeq=1015132