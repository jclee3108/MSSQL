IF OBJECT_ID('jongie_SSLInvoiceItemInfoQuery') IS NOT NULL
    DROP PROC jongie_SSLInvoiceItemInfoQuery
GO

-- v2013.09.11 

-- 거래명세서품목조회_jongie (품목조회) by이재천
 CREATE PROC jongie_SSLInvoiceItemInfoQuery    
 ...
 -- 수정종료
    ...
    SELECT (SELECT BizUnitName FROM _TDABizUnit WITH(NOLOCK) WHERE CompanySeq = @CompanySeq AND A.BizUnit = BizUnit)   AS BizUnitName,     --사업부문  
           ...
           E.Dummy6 AS FDiscountRate, 
           E.Dummy7 AS SDiscountRate, 
           G.CustPrice AS ConsumerPrice, 
            CASE WHEN ISNULL(G.CustPrice,0) = 0 
                 THEN 0 
                 ELSE (CASE WHEN B.Price IS NOT NULL
                            THEN B.Price
                            ELSE (ROUND(CASE WHEN ISNULL(B.Qty,0) = 0 THEN 0 ELSE (CASE WHEN B.IsInclusedVAT = '1' 
                                                                             THEN ((ISNULL(B.CurAmt,0) + ISNULL(B.CurVat,0)) / ISNULL(B.Qty,0))           
                                                                              ELSE (ISNULL(B.CurAmt,0) / ISNULL(B.Qty,0)) 
                                                                              END) END, @CurPriceLen)) END
                      ) / ISNULL(G.CustPrice,0) * 100 
                 END AS OutPercent, 
       CONVERT(DECIMAL(19,5),(B.STDQty / (L.ConvNum / L.ConvDen))) AS BoxUnitQty 
    
       FROM #Tmp_InvoiceProg           AS Z    
       JOIN _TSLInvoice                AS A WITH(NOLOCK) ON ( Z.InvoiceSeq = A.InvoiceSeq ) 
       JOIN _TSLInvoiceItem            AS B WITH(NOLOCK) ON ( A.CompanySeq = B.CompanySeq AND A.InvoiceSeq = B.InvoiceSeq AND Z.InvoiceSerl = B.InvoiceSerl ) 
       JOIN _TDASMinorValue            AS X WITH(NOLOCK) ON X.CompanySeq = @CompanySeq AND A.SMExpKind  = X.MinorSeq AND X.Serl = 1001 AND X.ValueText  = '1' 
       JOIN _TDAUMinorValue            AS OutK WITH(NOLOCK) ON ( A.CompanySeq = OutK.CompanySeq AND A.UMOutKind  = Outk.MinorSeq AND OutK.Serl = 2002 ) 
       LEFT OUTER JOIN _TDACust        AS F WITH(NOLOCK) ON ( A.CompanySeq = F.CompanySeq AND A.CustSeq = F.CustSeq ) 
       LEFT OUTER JOIN _TDAUMinor      AS H WITH(NOLOCK) ON ( A.CompanySeq = H.CompanySeq AND A.UMOutKind = H.MinorSeq ) 
       LEFT OUTER JOIN _TDAItem        AS J WITH(NOLOCK) ON ( B.CompanySeq = J.CompanySeq AND B.ItemSeq = J.ItemSeq ) 
       LEFT OUTER JOIN _TDAUMinorValue AS I WITH(NOLOCK) ON ( I.CompanySeq = H.CompanySeq AND I.MinorSeq = H.MinorSeq AND I.Serl = 2001 ) 
       LEFT OUTER JOIN (SELECT A.InvoiceSeq, A.InvoiceSerl, MAX(B.Seq) AS SalesSeq, MAX(B.Serl) AS SalesSerl, 
                       ...
                       )               AS U ON ( B.InvoiceSeq  = U.InvoiceSeq AND B.InvoiceSerl = U.InvoiceSerl ) 
       LEFT OUTER JOIN _TPJTProject    AS P WITH (NOLOCK) ON ( B.CompanySeq = P.CompanySeq AND B.PJTSeq = P.PJTSeq ) 
       LEFT OUTER JOIN _TPJTWBS        AS W WITH (NOLOCK) ON ( B.CompanySeq = W.CompanySeq AND B.PJTSeq = W.PJTSeq AND B.WBSSeq = W.WBSSeq ) 
       LEFT OUTER JOIN _TDACustGroup   AS O WITH(NOLOCK) ON( A.CompanySeq  = O.CompanySeq AND A.CustSeq = O.CustSeq AND O.UMCustGroup = 8014002 ) 
       LEFT OUTER JOIN _TDACustSalesReceiptCond AS Q WITH (NOLOCK) ON ( O.CompanySeq = Q.CompanySeq AND O.UpperCustSeq = Q.CustSeq AND O.CustSeq = Q.ReceiptCustSeq ) 
       LEFT OUTER JOIN _TDACust        AS R WITH(NOLOCK) ON ( R.CompanySeq = @CompanySeq AND O.UpperCustSeq = R.CustSeq ) 
       LEFT OUTER JOIN #TempResult     AS ZZ ON ( B.CompanySeq  = @CompanySeq AND B.InvoiceSeq  = ZZ.InOutSeq AND B.InvoiceSerl = ZZ.InOutSerl )  
       LEFT OUTER JOIN #TempSOSpec     AS S ON ( Z.OrderSeq  = S.OrderSeq AND Z.OrderSerl = S.OrderSerl ) 
       LEFT OUTER JOIN _TSLCustItem    AS T WITH(NOLOCK) ON ( T.CompanySeq = @CompanySeq AND T.ItemSeq = B.ItemSeq AND T.CustSeq = A.CustSeq AND T.UnitSeq = B.UnitSeq ) 
       LEFT OUTER JOIN _TDAItemAsset   AS C WITH(NOLOCK) ON ( C.CompanySeq = @CompanySeq AND C.AssetSeq = J.AssetSeq ) 
       JOIN #TCOMSourceTracking        AS D              ON ( D.IDX_NO = Z.IDX_NO ) 
       LEFT OUTER JOIN _TSLOrderItem   AS E WITH(NOLOCK) ON ( E.CompanySeq = @CompanySeq AND E.OrderSeq = D.Seq AND E.OrderSerl = D.Serl ) 
       LEFT OUTER JOIN _TSLItemUnitPrice AS G WITH(NOLOCK) ON ( G.CompanySeq = @CompanySeq AND G.ItemSeq = B.ItemSeq ) 
       LEFT OUTER JOIN _TDAItemUnit   AS L WITH(NOLOCK) ON ( L.CompanySeq = @CompanySeq AND L.UnitSeq = @EnvValue AND L.ItemSeq = B.ItemSeq ) 
    
     WHERE A.CompanySeq = @CompanySeq    
        AND (@PJTName = '' OR P.PJTName LIKE @PJTName + '%')  
        AND (@PJTNo   = '' OR P.PJTNo   LIKE @PJTNo   + '%')  
        AND (@WBSName = '' OR W.WBSName LIKE @WBSName + '%')  
        AND ((@SMProgress = '0' AND (@SMProgressType = 0 OR (Z.SMProgressType = @SMProgressType) OR ( @SMProgressType = 8098001 
                                                                                                  AND Z.SMProgressType IN (SELECT MinorSeq 
             ...
             ) OR (@SMProgress = '1' AND Z.SMProgressType IN (1037002, 1037003,1037001, 1037006))
            )
   ...
   RETURN
GO
exec jongie_SSLInvoiceItemInfoQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
...
</ROOT>',@xmlFlags=2,@ServiceSeq=1017680,@WorkingTag=N'',@CompanySeq=1,@LanguageSeq=1,@UserSeq=50322,@PgmSeq=1015132