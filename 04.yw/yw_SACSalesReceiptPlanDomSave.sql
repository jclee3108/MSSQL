IF OBJECT_ID('yw_SACSalesReceiptPlanDomSave') IS NOT NULL   
    DROP PROC yw_SACSalesReceiptPlanDomSave  
GO  
  
-- v2013.12.12  
  
-- 채권수금계획(내수)_yw-저장 by 이재천   
CREATE PROC yw_SACSalesReceiptPlanDomSave  
...
AS   
    ...
    BEGIN 
        ...
        DELETE B   
          ...
          JOIN yw_TACSalesReceiptPlan AS B ON ( B.CompanySeq = @CompanySeq AND A.PlanYM = B.PlanYM AND A.Serl = B.Serl AND B.PlanType = '1' )    
        ...
        DELETE B   
          FROM #yw_TACSalesReceiptPlan AS A   
          JOIN yw_TACSalesReceiptPlan AS B ON ( B.CompanySeq = @CompanySeq AND A.PlanYM = B.PlanYM AND B.PlanType = '1' )    
         WHERE A.WorkingTag = 'D'   
           AND A.Status = 0   
        END
    ...
    BEGIN    
        ...
        SELECT @CompanySeq , A.PlanYM, 1, A.Serl, A.DeptSeq, 
               A.CustSeq, 0, A.SMInType, A.PlanAmt, A.ReceiptAmt, 
               A.ReceiptAmt1, A.ReceiptAmt2, A.ReceiptAmt3, A.ReceiptAmt4, A.ReceiptAmt5, 
               A.ReceiptAmt6, A.LongBondAmt, A.BadBondAmt, 0, 0, 
               0, 0, 0, 0, 0, 
...
GO
begin tran
exec yw_SACSalesReceiptPlanDomSave @xmlDocument=N'<ROOT>
  <DataBlock1>
