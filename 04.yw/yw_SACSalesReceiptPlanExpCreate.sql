...
AS 
    ...
    SELECT A.EmpSeq, 
           ...
           SUM(ISNULL(CASE WHEN A.SMCustStatus <> 2004001 THEN A.OnForAmt ELSE 0 END,0)) AS BadBondDomAmt, -- 불량채권(원화)
           SUM(ISNULL(CASE WHEN A.SMCustStatus = 2004001 AND @PlanYM > LEFT(A.ReceiptDate,6) THEN A.OnForAmt ELSE 0 END,0)) AS LongBondDomAmt -- 장기채권(원화)
