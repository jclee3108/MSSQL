...
IF OBJECT_ID('DTI_SSLImpDelvItemListQuery') IS NOT NULL 
    DROP PROC DTI_SSLImpDelvItemListQuery
GO
 ...
 ************************************************************/
 CREATE PROC DTI_SSLImpDelvItemListQuery                
 ...
 AS        
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    EXEC _SCOMSourceTracking 
             ...
             @TempTableName = '#TMP',  -- 기준템프테이블
             @TempSeqColumnName = 'Seq',  -- 템프테이블 Seq
             @TempSerlColumnName = 'Serl',  -- 템프테이블 Serl
             @TempSubSerlColumnName = '' 

    SELECT A.Seq AS DelvSeq, B.Serl AS DelvSerl, B.Seq AS ApprovalReqSeq, B.Serl AS ApprovalReqSerl 
 ...
 /************************************************************************************************************************************************************************/
     ...
     **********************************************************/
     SELECT A.BizUnit,                                  --사업부문코드
            ...
            --B.Memo7                   AS Memo7,
            --B.Memo8                   AS Memo8, 
            EE.CustName AS SalesCustName, 
            RR.CustName AS EndUserName
       ...
       FROM #TMP AS D 
        ...
        LEFT OUTER JOIN _TDACurr C WITH(NOLOCK) ON A.CompanySeq = C.CompanySeq AND A.CurrSeq = C.CurrSeq
        LEFT OUTER JOIN _TDAItem       AS L WITH(NOLOCK) ON A.CompanySeq = L.CompanySeq AND B.ItemSeq = L.ItemSeq
        LEFT OUTER JOIN _TPJTProject   AS P WITH(NOLOCK) ON A.CompanySeq = P.CompanySeq AND B.PJTSeq = P.PJTSeq
        ...
        LEFT OUTER JOIN _TDASMinor     AS Z WITH(NOLOCK) ON Z.CompanySeq = @CompanySeq  AND K.SMProgressType = MinorSeq
        LEFT OUTER JOIN #TMP_DelvSource AS O ON D.Seq = O.DelvSeq AND D.Serl = O.DelvSerl  
     ...
     LEFT OUTER JOIN _TUIImpDelvCostDiv AS IH WITH(NOLOCK) ON A.CompanySeq = IH.CompanySeq 
        ...
        LEFT OUTER JOIN #TPUORDApprovalReqItem AS QQ WITH(NOLOCK) ON ( QQ.DelvSeq = D.Seq AND QQ.DelvSerl = D.Serl ) 
        LEFT OUTER JOIN _TPUORDApprovalReqItem AS WW WITH(NOLOCK) ON ( WW.CompanySeq = @CompanySeq AND WW.ApproReqSeq = QQ.ApprovalReqSeq AND WW.ApproReqSerl = QQ.ApprovalReqSerl ) 
        LEFT OUTER JOIN _TDACust               AS EE WITH(NOLOCK) ON ( EE.CompanySeq = @CompanySeq AND EE.CustSeq = CONVERT(INT,WW.Memo1) ) 
        LEFT OUTER JOIN _TDACust               AS RR WITH(NOLOCK) ON ( RR.CompanySeq = @CompanySeq AND RR.CustSeq = CONVERT(INT,WW.Memo2) ) 
...
GO
exec DTI_SSLImpDelvItemListQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
    ...
    <TopUnitName />
    <BizUnit>1</BizUnit>
    <DelvDateFr>20100101</DelvDateFr>
    <DelvDateTo>20131230</DelvDateTo>
    <CustSeq />
    <SMImpKind />
    <SMImpKindName />
    <DelvNo />
...
</ROOT>',@xmlFlags=2,@ServiceSeq=4531,@WorkingTag=N'',@CompanySeq=1,@LanguageSeq=1,@UserSeq=50322,@PgmSeq=5564