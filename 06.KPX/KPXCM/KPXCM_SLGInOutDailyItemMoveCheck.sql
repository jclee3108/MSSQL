IF OBJECT_ID('KPXCM_SLGInOutDailyItemMoveCheck') IS NOT NULL 
    DROP PROC KPXCM_SLGInOutDailyItemMoveCheck
GO 
 ...
 ************************************************************/        
 CREATE PROC KPXCM_SLGInOutDailyItemMoveCheck        
 ...
 AS               
    ...
    ---------------------------------------------------------------------------
     ...
     ----창고재고실사등록에서 생성 된 건은 기타출고 입력에서 수정,삭제 될 수 없도록
     
     --   IF EXISTS (SELECT * FROM #TLGInOutDailyItem WHERE WorkingTag IN ('U', 'D') AND @PgmSeq = 1368)
     --BEGIN
     --    --메세지 받아오기
     ...
     --           Status        = @Status
     --      FROM #TLGInOutDailyItem AS A
     --           JOIN _TLGInOutDailyItem AS B ON @CompanySeq = B.CompanySeq
     ...
     UPDATE #TLGInOutDailyItem    
      ...
      WHERE A.WorkingTag = 'U'
        ...
        AND A.InOutType  = 30
        AND EXISTS ( SELECT 1 FROM _TLGWHStkReal WHERE InOutSeq = A.InOutSeq AND IsEtcOut = '1' AND CompanySeq = @CompanySeq )
        AND (A.ItemSeq         <> B.ItemSeq  OR
             A.UnitSeq         <> B.UnitSeq  OR
             A.Qty             <> B.Qty      OR
  ...
  UPDATE A
   ...
   IF @@ROWCOUNT = 0 OR ISNULL( @EnvValue, '' ) = '' SELECT @EnvValue = '' 
     ...
     DECLARE @ItemNo NVARCHAR(100)
      ...
      IF @@ROWCOUNT = 0
     BEGIN
         SELECT @ItemNo = ( CASE @LanguageSeq WHEN 1 THEN '품번' ELSE 'ItemNo' END )
     END
       -------------------------------------------        
      -- Lot관리시 Lot필수체크체크        
      -------------------------------------------        
      EXEC dbo._SCOMMessage @MessageType OUTPUT,        
                            @Status      OUTPUT,        
                            @Results     OUTPUT,        
                            1171               , -- 해당품목은 Lot번호 관리 품목입니다. Lot번호를 필수로 입력하세요.(SELECT * FROM _TCAMessage WHERE MessageSeq = 1171)        
                            @LanguageSeq       ,         
                            0, ''     
       UPDATE #TLGInOutDailyItem        
         SET Result        = @Results + ' (' + @ItemNo + ' : ' + I.ItemNo + ')',        
             MessageType   = @MessageType,        
             Status        = @Status        
        FROM #TLGInOutDailyItem AS A
             JOIN ( SELECT X.InOutType, X.InOutSeq, X.InOutSerl
                      FROM #TLGInOutDailyItem              AS X
                           LEFT OUTER JOIN _TLGInOutLotSub AS Y WITH(NOLOCK) ON Y.CompanySeq    = @CompanySeq
                                                           AND X.InOutType     = Y.InOutType
                                                                            AND X.InOutSeq      = Y.InOutSeq
                                                                            AND X.InOutSerl     = Y.InOutSerl
                   AND X.InOutDataSerl = 0
                    GROUP BY X.InOutType, X.InOutSeq, X.InOutSerl ) AS B ON B.InOutType = A.InOutType
             ...
             JOIN _TDAItemStock AS C ON C.CompanySeq    = @CompanySeq 
                                    ...
                                    AND C.IsLotMng      = '1'
             JOIN _TDAItem      AS I ON I.CompanySeq    = @CompanySeq 
  ...
  /***************** 기준단위 체크 ************************************/  
     ...
     BEGIN            
         ...
         SELECT @Seq = ISNULL((SELECT MAX(A.InOutSerl)          
                                 FROM _TLGInOutDailyItem AS A WITH(NOLOCK) JOIN _TLGInOutDaily AS A10  WITH(NOLOCK)         
