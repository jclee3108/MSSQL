...
CREATE PROCEDURE KPXCM_SLGInOutReqGWQuery    
 ...
 AS         
    ...
    SELECT ROW_NUMBER() OVER(ORDER BY A.ReqSeq) AS Num, 
           ...
           REPLACE(CONVERT(NVARCHAR,CONVERT(money, LEFT(CONVERT(DECIMAL(19,4),B.Qty),charindex('.',CONVERT(DECIMAL(19,4),B.Qty)) -1 )),1),'.00','') + '.'+ 
           RIGHT(CONVERT(DECIMAL(19,4),B.Qty), LEN(CONVERT(DECIMAL(19,4),B.Qty)) - charindex('.',CONVERT(DECIMAL(19,4),B.Qty)) ) AS Qty 
...
EXEC _SCOMGroupWarePrint 2, 1, 1, 1358, 'EtcOutReq_CM', '116', ''