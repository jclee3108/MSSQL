IF OBJECT_ID('KPXCM_SPDSFCLotNoCheckS2Query') IS NOT NULL 
    DROP PROC KPXCM_SPDSFCLotNoCheckS2Query
GO 

-- v2016.04.20 
-- Lot오류검사-오류조회 by 전경만   
CREATE PROC KPXCM_SPDSFCLotNoCheckS2Query
...
AS   
    ...
    SELECT A.ProdPlanSeq,
           ...
           A.ItemSeq,
           A.ProdQty,
           ROW_NUMBER() OVER(ORDER BY ItemSeq, SrtDate, WorkCond1) AS IDX
      INTO #ProdPlan
     ...
     ORDER BY ItemSeq, SrtDate, WorkCond1
    ...
    SELECT ItemSeq,
           ...
           WorkCond1,
           WorkCond3,
           ROW_NUMBER() OVER(ORDER BY ItemSeq, WorkCond3) AS LotIDX
      INTO #ProdLot
      FROM #ProdPlan
     ORDER BY ItemSeq, WorkCond3
--select * from #ProdPlan
    SELECT A.ProdPlanSeq, 
      ...
      FROM #ProdPlan AS A
           JOIN #ProdLot AS B ON B.ProdPlanSeq = A.ProdPlanSeq AND A.IDX <> B.LotIDX
           LEFT OUTER JOIN _TDAItem AS I WITH(NOLOCK) ON I.CompanySeq = @CompanySeq
                                                     AND I.ItemSeq = A.ItemSeq
           LEFT OUTER JOIN _TPDROUItemProcWC AS C WITH(NOLOCK) ON C.CompanySeq = @CompanySeq
...
exec KPXCM_SPDSFCLotNoCheckS2Query @xmlDocument=N'<ROOT>
  <DataBlock1>
