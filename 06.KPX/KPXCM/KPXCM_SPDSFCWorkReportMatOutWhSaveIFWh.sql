IF OBJECT_ID('KPXCM_SPDSFCWorkReportMatOutWhSaveIFWh') IS NOT NULL 
    DROP PROC KPXCM_SPDSFCWorkReportMatOutWhSaveIFWh
GO 

-- v2016.11.16  

-- 생산실적자재투입 불출창고 로직 추가 by이재천 
 ...
 ************************************************************/   
 CREATE PROC KPXCM_SPDSFCWorkReportMatOutWhSaveIFWh
 ...
 AS          
    ...
    -- I/F 테이블 창고 셋팅하기 위해 담기 2016.11.14 by이재천 
    SELECT WorkReportSeq, ItemSerl, MatItemSeq, WHSeq 
      INTO #IFWHSeq 
      FROM #TPDSFCMatinput 
...
--            EXEC _SLGInOutMinusCheck @CompanySeq, '#TLGInOutDailyBatch'      
        ...
        INSERT INTO #ItemWH ( WorkReportSeq, ItemSerl, ItemSeq, OutWHSeq, FactUnit, AssetSeq, WorkCenterSeq )
        SELECT DISTINCT A.WorkReportSeq, A.ItemSerl, A.MatItemSeq AS ItemSeq, ISNULL(B.OutWHSeq, M.OutWHSeq) AS OutWHSeq, M.FactUnit, D.AssetSeq, E.WorkCenterSeq
          FROM #MatAutoOut                  AS A 
                     JOIN _TPDMMOutM        AS M ON ( M.CompanySeq = @CompanySeq AND A.WorkReportSeq = M.WorkReportSeq ) 
          ...
          LEFT OUTER JOIN _TPDSFCWorkReport AS E ON ( E.CompanySeq = @CompanySeq AND E.WorkReportSeq = A.WorkReportSeq ) 
        


        -- 제품의 불출창고가 원부료창고로 나가는 오류가 발생하는데 원인 파악이 되지 않아 나중에 확인하기 위해 담아둠
        --CREATE TABLE TempItemWH_20160919
        --(
        --    IDX_NO          INT IDENTITY, 
        --    WorkReportSeq   INT, 
        --    ItemSeq         INT, 
        ...
        --)
        --INSERT INTO TempItemWH_20160919 ( WorkReportSeq, ItemSeq, OutWHSeq, FactUnit, AssetSeq, WorkCenterSeq, WorkDateTime )
        --SELECT *, GETDATE()
        --  FROM #ItemWH
        ...
        UPDATE A
          ...
          JOIN ( 
                SELECT A.ValueSeq AS OutWHSeq, B.ValueSeq AS FactUnit, C.ValueSeq AS AssetSeq
                  ...
                  LEFT OUTER JOIN _TDAUMinorValue AS B ON ( B.CompanySeq = A.CompanySeq AND B.MinorSeq = A.MinorSeq AND B.Serl = 1000001 ) -- 생산사업장 
                  LEFT OUTER JOIN _TDAUMinorValue AS C ON ( C.CompanySeq = A.CompanySeq AND C.MinorSeq = A.MinorSeq AND C.Serl = 1000002 ) -- 품목자산분류 
                  LEFT OUTER JOIN _TDAUMinorValue AS D ON ( D.CompanySeq = A.CompanySeq AND D.MinorSeq = A.MinorSeq AND D.Serl = 1000004 ) -- 워크센터
                 WHERE A.CompanySeq = @CompanySeq 
                   AND A.Serl = 1000003 -- 불출창고 
                   AND A.MajorSeq = 1012905
                   AND ISNULL(D.ValueSeq,0) = 0 
               ) AS Y ON ( Y.AssetSeq = A.AssetSeq AND Y.FactUnit = A.FactUnit ) 
        */

        -- I/F테이블의 창고로 셋팅 2016.11.15 by이재천 

        -- 사용자정의코드 - 품목자산분류별 불출기본창고
        -- 기본 불출창고 셋팅 
        SELECT A.ItemSeq, E.ValueSeq AS OutWHSeq 
          INTO #OutWHSeq 
          FROM _TDAItem                     AS A WITH(NOLOCK) 
                     JOIN #IFWHSeq          AS B              ON ( B.MatItemSeq = A.ItemSeq ) 
        ...
        -- 연동으로 넘어오지 않은 창고는 사용자정의코드에 정의되어 있는 불출창고로 적용 
        UPDATE A
           SET WHSeq = B.OutWHSeq 
          FROM #IFWHSeq                     AS A 
          LEFT OUTER JOIN #OutWHSeq         AS B ON ( B.ItemSeq = A.MatItemSeq ) 
         WHERE ISNULL(A.WHSeq,0) = 0 
        
        UPDATE A
           SET OutWHSeq = B.WHSeq  
          FROM #ItemWH  AS A 
          JOIN #IFWHSeq AS B ON ( B.WorkReportSeq = A.WorkReportSeq AND B.ItemSerl = A.ItemSerl )
        
        
        -- 연구소 워크센터일 경우 연구소 창고로 셋팅 
        UPDATE A
           SET OutWHSeq = Y.OutWHSeq 
          FROM #ItemWH AS A 
          JOIN ( 
                SELECT A.ValueSeq AS OutWHSeq, B.ValueSeq AS FactUnit, C.ValueSeq AS AssetSeq, ISNULL(D.ValueSeq,0) AS WorkCenterSeq 
                  FROM _TDAUMinorValue AS A 
                  LEFT OUTER JOIN _TDAUMinorValue AS B ON ( B.CompanySeq = A.CompanySeq AND B.MinorSeq = A.MinorSeq AND B.Serl = 1000001 ) -- 생산사업장 
                  LEFT OUTER JOIN _TDAUMinorValue AS C ON ( C.CompanySeq = A.CompanySeq AND C.MinorSeq = A.MinorSeq AND C.Serl = 1000002 ) -- 품목자산분류 
                  LEFT OUTER JOIN _TDAUMinorValue AS D ON ( D.CompanySeq = A.CompanySeq AND D.MinorSeq = A.MinorSeq AND D.Serl = 1000004 ) -- 워크센터
                 WHERE A.CompanySeq = @CompanySeq 
                   ...
                   AND A.MajorSeq = 1012905
                   AND ISNULL(D.ValueSeq,0) <> 0 
               ) AS Y ON ( Y.AssetSeq = A.AssetSeq AND Y.FactUnit = A.FactUnit AND A.WorkCenterSeq = Y.WorkCenterSeq ) 
        
        
        --CREATE TABLE TempItemWH_20160919_2
        --(
        --    IDX_NO          INT IDENTITY, 
        --    WorkReportSeq   INT, 
        --    ItemSeq         INT, 
        ...
        --    WorkDateTime    DATETIME 
        --)
        --INSERT INTO TempItemWH_20160919_2 ( WorkReportSeq, ItemSeq, OutWHSeq, FactUnit, AssetSeq, WorkCenterSeq, WorkDateTime )
        --SELECT *, GETDATE()
        --  FROM #ItemWH
        -----------------------------------------------------
        -- KPXCM용 사용자정의코드 불출창고 셋팅, END 
        -----------------------------------------------------
         INSERT INTO  _TPDMMOutItem      
             ...
             SELECT @CompanySeq      ,M.MatOutSeq        ,A.ItemSerl         ,A.MatItemSeq       ,ISNULL(W.OutWHSeq,0), 
               ...
               FROM #MatAutoOut          AS A         
                 ...
                 LEFT OUTER JOIN #ItemWH AS W ON ( W.WorkReportSeq = A.WorkReportSeq AND W.ItemSeq = A.MatItemSeq AND W.ItemSerl = A.ItemSerl )    
