IF OBJECT_ID('KPXCM_SPDWorkOrderCfmPilotSave') IS NOT NULL 
    DROP PROC KPXCM_SPDWorkOrderCfmPilotSave
GO 

-- v2016.03.02 

-- 수정 by이재천 작업지시에 동품이명코드 넣기 (WorkCond5) 
-- 긴급작업지시때문에 별도 SP생성 
CREATE PROC KPXCM_SPDWorkOrderCfmPilotSave
 ...
 --select * from #TPDSFCWorkOrder  
    ...
    INSERT INTO #TPDMPSDailyProdPlan (ProdPlanSeq, FactUnit, SrtDate, EndDate,WorkCenterSeq,QcType,WorkCond7)   
    SELECT A.ProdPlanSeq, A.FactUnit,   
           CONVERT(DATETIME, A.SrtDate+SPACE(1)+LEFT(A.WorkCond1,2)+':'+RIGHT(A.WorkCond1,2)),  
           CONVERT(DATETIME, A.EndDate+SPACE(1)+LEFT(A.WorkCond2,2)+':'+RIGHT(A.WorkCond2,2)),  
           A.WorkCenterSeq,  
           (select max(Q.QcType)  
              from  KPX_TQCQAProcessQCType AS Q WITH(NOLOCK) where Q.CompanySeq = @CompanySeq    
                                                               AND Q.ProcQC = 1000498001  
                                                               AND Q.QCType IN (
                                                                                   SELECT DISTINCT QCType 
                                                                                     FROM KPX_TQCQASpec as Q1 
                                                                                    WHERE Q1.CompanySeq = @CompanySeq  
                                                                                      AND Q1.ItemSeq = A.ItemSeq
                                                                               )  
           ), 
           B.AfterWorkSeq 
     ...
     WHERE A.CompanySeq = @CompanySeq     
        ...
        and ( Isnull(B1.IsLOtMng,'0') = '0' or (B1.IsLotMng = '1' and A.WorkCond3 <> '') )  
     ...
     IF EXISTS (SELECT 1     
                  ...
                  LEFT OUTER JOIN #TPDMPSDailyProdPlan AS B ON ( A.WorkCenterSeq = B.WorkCenterSeq )    
     ...
     (    
         ...
         WorkCond1   NVARCHAR(500),     --시작일  
         WorkCond2   NVARCHAR(500),     --종료일  
         SrtTime     NCHAR(4),     
         EndTime     NCHAR(4),  
         WorkCond3   NVARCHAR(100),  
         WorkCenterSeq INT,  
         WorkCond6   INT,  
         Remark      NVARCHAR(200), 
     ...
     INSERT INTO @DailyProdPlan (    
                                 ...
                                 EndDate,DeptSeq, WorkCond1, WorkCond2,  
                                 SrtTime, EndTime, WorkCond3, WorkCenterSeq, WorkCond6,Remark,WorkCond5, WorkCond7
                                )     
     SELECT B.FactUnit, A.ProdPlanSeq, A.ProdPlanNo, A.SrtDate, A.ItemSeq,     
            A.BOMRev, A.ProcRev, A.ProdQty, A.UnitSeq, A.SMSource,     
            A.EndDate, A.DeptSeq, A.SrtDate, A.EndDate, A.WorkCond1, A.WorkCond2, A.WorkCond3, A.WorkCenterSeq, A.WorkCond6,A.Remark, A.WorkCond5, B.WorkCond7
     ...
     --                                                   AND C.WorkcenterSeq = X.WorkCenterSeq   
     -- WHERE A.ComPanySeq = @ComPanySeq          
     --   AND B.ProcSeq <> 0          
     ...
     */    
     ...
     SELECT DISTINCT A.FactUnit, X.ItemSeq,    X.BOMRev,   X.ProcRev,     B.ProcSeq,  C.WorkCenterSeq, B.ProcNo,        
       ...
       JOIN _TPDROUItemProcWC          AS C WITH(NOLOCK) ON A.ItemSeq = C.ItemSeq           
                                                        ...
                                                        AND B.ComPanySeq = C.ComPanySeq           
                                                        AND C.FactUnit = A.FactUnit       
                                                        and X.WorkCenterSeq = C.WorkCenterSeq  
     ...
     --select '1',* from @ItemProcInfo  
     --select * from @DailyProdPlan  
     --  return  
     UPDATE @ItemProcInfo    
   ...
   --    select * from @ItemProcAssy  
    ...
    --return  
     ...
     SELECT      
             ...
             A.WorkCond7 
       FROM @DailyProdPlan AS A     
       JOIN @ItemProcInfo AS B ON A.ItemSeq = B.ItemSeq AND A.BomRev = B.BomRev AND A.ProcRev = B.ProcRev And A.WorkCenterSeq = B.WorkCenterSeq     
       ...
       LEFT OUTER JOIN _TPDROUItemProcWC  AS D ON B.ItemSeq = D.ItemSeq     
                                         ...
                                         AND B.ProcSeq = D.ProcSeq     
                                              ...
                                              AND D.Ranking = 1    
                                              AND D.CompanySeq = @CompanySeq     
       LEFT OUTER JOIN _TPDBaseWorkcenter AS F ON  A.WorkcenterSeq = F.WorkcenterSeq     
...
--select '1',* from @DailyProdPlan  
--select * from @ItemProcInfo  
--select * from @ItemProcAssy  
  
--select * from _TDAItem where companyseq = 1 and itemseq IN (2055,2065)  
--select '2',* from #TPDMPSWorkOrder  
--return  
     ...
     BEGIN          
         ...
         BEGIN        
             ...
             BEGIN          
                ...
                EXEC _SCOMCreateNo 'PD', '_TPDSFCWorkOrder', @CompanySeq, @FactUnit, @WorkDate, @WorkOrderNo OUTPUT    
--select @WorkORderNo  
...
--select @WorkORderNo  
     ...
     (          
         ...
         GoodItemSeq,  ProcSeq,       AssyItemSeq,   ProdUnitSeq,         
         OrderQty,          
         StdUnitQty,     
         WorkDate,      WorkStartTime, WorkEndTime,       ChainGoodsSeq,          
         ...
         LastDateTime, BatchSeq,        
         WorkCond1,  WorkCond2, WorkCond3, WorkCond6, WorkCond5, WorkCond7 
     )          
     SELECT               
             ...
             GoodItemSeq,  ProcSeq,          ISNULL(AssyItemSeq,0), ProdUnitSeq,         
             CASE WHEN B.SMDecPointSeq = 1003001 THEN ROUND(OrderQty, @QtyPoint, 0)   -- 2011. 4. 19 hkim 소수점 자리수 통제하기 위해서 수정--CONVERT(DECIMAL(19,5),OrderQty),            
             ...
             A.LastDateTime, A.BatchSeq,       
             A.WorkCond1, A.WorkCond2, A.WorkCond3, A.WorkCond6, A.WorkCond5, A.WorkCond7  --,A.Remark  
       FROM #TPDMPSWorkOrder AS A        
       JOIN _TDAUnit          AS B ON A.CompanySeq = B.CompanySeq And A.ProdUnitSeq = B.UnitSeq    -- 2011. 4. 19 hkim 단위 등록의 소수점 계산방식을 가져오기 위해      
       ...
       LEFT OUTER JOIN (
                                    SELECT  ValueSeq    AS ItemSeq
                                      FROM _TDAUMinorValue
                                     WHERE CompanySeq = @CompanySeq
                                       AND MajorSeq = 1011291
                                       AND Serl = 1000001
                       )      AS E ON A.GoodItemSeq = E.ItemSeq
      WHERE ISNULL(WorkOrderNo ,'') <> ''             
        ...
        AND (CASE WHEN E.ItemSeq IS NULL THEN A.OrderQty ELSE 1 END) <> 0
     ...
     (    
         ...
         WorkOrderDate,  
         ProdPlanSeq,WorkPlanSerl,DailyWorkPlanSerl,WorkCenterSeq,    
         ...
         IsCancel, LastUserSeq, LastDateTime, BatchSeq,   
         WorkCond1, WorkCond2, WorkCond3, WorkCond6, WorkCond5, WorkCond7 
     )     
     SELECT A.CompanySeq, A.WorkOrderSeq, A.FactUnit, A.WorkOrderNo, A.WorkOrderSerl,    
            CONVERT(NCHAR(8), CONVERT(DATETIME, GETDATE()),112), -- 작업지시일을 기존 생산계획완료일이 아닌 생산계획 확정일자로 변경 2012.06.18 by 허승남    
            A.ProdPlanSeq, A.WorkPlanSerl, A.DailyWorkPlanSerl, A.WorkCenterSeq,   
            A.GoodItemSeq, A.ProcSeq, A.AssyItemSeq, A.ProdUnitSeq, A.OrderQty,   
            A.StdUnitQty, A.WorkDate, A.WorkStartTime, A.WorkEndTime, A.ChainGoodsSeq,    
            6041001, A.DeptSeq, A.ItemUnitSeq, A.ProcRev, A.Remark,    
            A.IsProcQC,--IsProcQC,    
            A.IsLastProc,--IsLastProc,    
            A.IsPjt, A.PjtSeq, A.WBSSeq,    
            A.ItemBomRev, A.ProcNo, A.ToProcNo, A.SMToProcMovType, 0,    
            '0',    
            A.LastUserSeq,    
            A.LastDateTime,    
            A.BatchSeq,    
            A.WorkCond1, A.WorkCond2, A.WorkCond3, A.WorkCond6, A.WorkCond5, A.WorkCond7 
         FROM _TPDMPSWorkOrder AS A     
    ...
    SELECT @DeptSeq = MAX(W.DeptSeq)  
      FROM _TPDMPSWorkOrder AS A  
           JOIN #TPDMPSDailyProdPlan AS B ON B.ProdPlanSeq = A.ProdPlanSeq  
           JOIN _TPDBaseWorkCenter AS W ON W.CompanySeq = @CompanySeq  
                                       AND W.WorkCenterSeq = A.WorkCenterSeq  
                                       AND W.OutMatLeadTime <> '1'  
                                       AND W.CapaRate > 0  
     WHERE A.CompanySeq = @CompanySeq  
...
--GO
BEGIN TRAN
exec KPXCM_SPDWorkOrderCfmSave @xmlDocument=N'<ROOT>
  <DataBlock1>
    ...
    <Selected>1</Selected>
    <Status>0</Status>
    <FrStdDate>20161202</FrStdDate>
    <FrTime>0800</FrTime>
    <ToStdDate>20161204</ToStdDate>
    <ToTime>1200</ToTime>
    <WorkCenterName />
    <WorkCenterSeq>0</WorkCenterSeq>
