IF OBJECT_ID('KPXCM_SPUDelvCheck') IS NOT NULL 
    DROP PROC KPXCM_SPUDelvCheck
GO 
 ...
 ************************************************************/  
 ...
 CREATE PROC KPXCM_SPUDelvCheck    
...
-----------------------------------------------------------------------------------
    -- 체크, MES 연동처리 된 데이터는 삭제 할 수 없습니다. by이재천 
    -----------------------------------------------------------------------------------
    ...
    INSERT INTO #ProgData ( DelvSeq, DelvSerl ) 
    ...
    EXEC _SCOMSourceTracking @CompanySeq = @CompanySeq, 
                             ...
                             @TempTableName = '#ProgData',  -- 기준템프테이블
    ...
    UPDATE A 
       SET Result = 'MES 연동처리 된 데이터는 삭제 할 수 없습니다.', 
           Status = 1234, 
           MessageType = 1234 
      FROM #TPUDelv             AS A 
      JOIN #ProgData            AS B ON ( B.DelvSeq = A.DelvSeq ) 
      JOIN #TCOMSourceTracking  AS C ON ( C.IDX_NO = B.IDX_NO ) 
     WHERE EXISTS (SELECT 1 FROM IF_PUInQCResult_MES WHERE CompanySeq = @CompanySeq AND POSeq = C.Seq AND POSerl = C.Serl) 
       AND A.Status = 0 
       AND A.WorkingTag = 'D' 
    -----------------------------------------------------------------------------------
    -- 체크, END 
