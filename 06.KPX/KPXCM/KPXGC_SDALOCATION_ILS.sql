IF OBJECT_ID('KPXGC_SDALOCATION_ILS') IS NOT NULL 
    DROP PROC KPXGC_SDALOCATION_ILS
GO 
...
/************************************************************
 ...
 작성자 - 전경만
************************************************************/
CREATE PROC KPXGC_SDALOCATION_ILS
...
AS
    ...
    INSERT INTO KPXCM.dbo.LOCATION_IF 
    SELECT LOC_CD, LOC_NM, WORK_FLAG, SEND_FLAG
      FROM LOCATION_IF
     WHERE Send_Flag = '0'
    
    /*
    -- KPX라이프사이언스 DB 데이터 연동 -- 2015.09.15 이재천 추가 
    INSERT INTO KPXLS.dbo.LOCATION_IF 
    SELECT LOC_CD, LOC_NM, WORK_FLAG, SEND_FLAG
      FROM LOCATION_IF
     WHERE Send_Flag = '0'
     
    -- KPX홀딩스 DB 데이터 연동 -- 2015.09.15 이재천 추가 
    INSERT INTO KPXHD.dbo.LOCATION_IF 
    SELECT LOC_CD, LOC_NM, WORK_FLAG, SEND_FLAG
      FROM LOCATION_IF
     WHERE Send_Flag = '0'
    */
