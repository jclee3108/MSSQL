IF OBJECT_ID('backup_20151001_KPX_SLGInOutDailySave') IS NOT NULL 
    DROP PROC backup_20151001_KPX_SLGInOutDailySave
GO 
...
CREATE PROC backup_20151001_KPX_SLGInOutDailySave      
...
AS                  
    ...
    SELECT @TableColumns = dbo._FGetColumnsForLog('KPX_TLGInOutDailyAdd')      
 ...
 */      
  ...
  */      
        ...
        -- 입출고Master 추가정보 DELETE                
  
        DELETE KPX_TLGInOutDailyAdd                  
  
          FROM #TLGInOutDaily AS A                  
  
               JOIN KPX_TLGInOutDailyAdd AS B WITH (NOLOCK) ON B.CompanySeq  = @CompanySeq                  
...
--                InOutType     = A.InOutType,              
        ...
        UPDATE KPX_TLGInOutDailyAdd                  
  
           SET  WOReqSeq   = ISNULL(A.WOReqSeq,0),       
  
                WOReqSerl     = ISNULL(A.WOReqSerl,0),      
  
                Kind          = ISNULL(A.Kind,0),   
   ...
   -- 창고를 _TLGInOutStock에 반영하기 ( 적송처리,자재적송처리에만 한하여 ) - 2011.09.21 by 김철웅, 1)       
    ...
    BEGIN                  
      ...
      OutWHSeq,              
        ...
        INSERT INTO KPX_TLGInOutDailyAdd(                  
