IF OBJECT_ID('KPXGC_SPDSFCMatInput_POP') IS NOT NULL 
    DROP PROC KPXGC_SPDSFCMatInput_POP
go 
/*************************************************************************************************************    
 설  명 - ERP 투입실적 생성 : 생산실적이 먼저 처리 되고, 투입데이터가 나중에 처리되는 경우 
 작성일 - 20151123
 작성자 - 천혜연   
*************************************************************************************************************/    
CREATE PROCEDURE KPXGC_SPDSFCMatInput_POP
    @CompanySeq  INT = 1             -- 법인          
AS        
   ...
   -- and A.RegEmpSeq <> 1064  
    ...
    UPDATE A       
      ...
      FROM KPX_TPDSFCWorkReportExcept_POP   AS A 
      JOIN #ErrorTemp                       AS B ON ( B.CompanySeq = A.CompanySeq AND B.Seq = A.Seq ) 
    ...
    UPDATE A
       SET WHSeq = CASE WHEN S.SMAssetGrp = 6008006 THEN B.MatOutWhSeq ELSE A.FieldWHSeq END
      ...
      JOIN _TDAItemAsset      AS S WITH(NOLOCK) ON S.CompanySeq = @CompanySeq    
...
--select @XmlData      
    ...
    IF EXISTS (SELECT 1 FROM #TLGInOutDailyBatch WHERE Status <> 0)          
     BEGIN          
        UPDATE KPX_TPDSFCWorkReportExcept_POP          
           SET ProcYN = '2',        
               ErrorMessage = 'KPX_SLGInOutDailyBatch_POP 오류',        
               ProcDateTime = GETDATE(),        
    ...
    --             JOIN #TMP_PDSFCMatinput_Xml            AS C ON ( C.Seq = A.Seq ) 
    --  LEFT OUTER JOIN KPX_TPDSFCWorkReportExcept_POP    AS B ON ( B.CompanySeq = @CompanySeq AND B.Seq = A.Seq ) 
    -- WHERE A.CompanySeq = @CompanySeq 
    --   AND ( ISNULL(B.WorkReportSeq,0) <> 0 AND ISNULL(B.ItemSerl,0) <> 0 ) 
