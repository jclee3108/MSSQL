...
IF OBJECT_ID('KPX_SACDailyExpPlanItemCheck') IS NOT NULL   
    DROP PROC KPX_SACDailyExpPlanItemCheck  
GO  
  
-- v2014.12.09  
  
-- 일일외화매각계획서-매각 체크 by 이재천   
CREATE PROC KPX_SACDailyExpPlanItemCheck  
...
AS   
    ...
    */
    ...
    SELECT @MaxSerl = (SELECT MAX(Serl) FROM KPX_TACDailyExpPlanItem WHERE CompanySeq = @CompanySeq AND BaseDate = (SELECT TOP 1 BaseDate FROM #KPX_TACDailyExpPlanItem WHERE WorkingTag = 'A'))
