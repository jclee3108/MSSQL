...
IF OBJECT_ID('KPX_SACFundListSubQuery') IS NOT NULL   
    DROP PROC KPX_SACFundListSubQuery  
GO  
  
-- v2015.05.06 
  
-- 상품운용현황-조회 by 이재천   
CREATE PROC KPX_SACFundListSubQuery  
...
AS  
    ...
    SELECT --*  
      ...
      OUTER APPLY (SELECT SUM(Z.CancelAmt) AS CancelAmt  
                 ...
                 JOIN _FCOMXmlToSeq(@UMHelpCom, @MultiUMHelpCom) AS E ON ( A.UMHelpCom = @UMHelpCom OR E.Code = A.UMHelpCom ) 
      LEFT OUTER JOIN _TDAUMinor AS F ON ( F.CompanySeq = @CompanySeq AND F.MinorSeq = A.UMHelpCom ) 
     WHERE A.StdDate <= @StdDate   
       ...
       --AND (A.UMHelpCom IN ( SELECT Z.MinorSeq   
       --                       FROM _TDAUMinor AS Z   
       --                       JOIN _TDAUMinorValue AS Y ON ( Y.CompanySeq = @CompanySeq AND Y.MinorSeq = Z.MinorSeq AND Y.ValueText = CONVERT(NVARCHAR(3),@CompanySeq) )   
       --                      WHERE Z.CompanySeq = @CompanySeq   
       --                        AND Z.MajorSeq= 1010494   
       --                    )   
       --    )  
    ...
    (  
        ...
        FundKindSSeq    , FundSeq         , DurDate         , DiffActDate     , UMHelpComName
    )   
    SELECT A.FundName,   
      ...
      FROM (  
              SELECT DISTINCT FundName, FundSeq, UMHelpCom, FundKindS, FundKindM, UMBond, TitileName, UMHelpComName  -- , SliptAmt , ResultReAmt, SumSliptAmt, SumResultReAmt  
              FROM #BaseData   
           )AS A   
      ...
      OUTER APPLY ( SELECT Z.TestAmt, Z.ActAmt, Z.PrevAmt, Z.InvestAmt, Z.AddAmt, Z.SrtDate, Z.DurDate, Z.DiffActDate
                      FROM #BaseData AS Z     
    ...
    UPDATE A
       SET ChProfitRate     = ISNULL(A.AddAmt,0), -- 연수익율
           PeProfitRate     = (ISNULL(A.AddAmt,0) * (DATEDIFF(DAY,A.SrtDate, @StdDate))) / 365, -- 연환산수익율(가입일이후)*(기준일자-최초 가입일자)/365
           SumProfitRate    = (ISNULL(A.AddAmt,0) * (DATEDIFF(DAY,@SubStdDate, @StdDate))) / 365, -- 연환산수익율(가입일이후)*(평가일자-기준일자)/365
...
GO 
    ...
    RETURN  
GO 
exec KPX_SACFundListSubQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
    ...
    <SubStdDate>20150101</SubStdDate>
    <StdDate>20150515</StdDate>
    <UMHelpCom>1010494004</UMHelpCom>
    <UMBond />
    <FundName />
  </DataBlock1>
