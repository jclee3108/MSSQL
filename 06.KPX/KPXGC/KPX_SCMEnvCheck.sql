...
IF OBJECT_ID('KPX_SCMEnvCheck') IS NOT NULL 
    DROP PROC KPX_SCMEnvCheck
GO 

-- v2014.08.25    
     
 -- 환경설정(KPX)-체크 by 서보영    Save as by이재천
 CREATE PROC KPX_SCMEnvCheck      
 ...
 AS      
   ...
   SELECT @Count = Count(1) FROM #TCOMEnv WHERE WorkingTag = 'A' AND Status = 0
   IF @Count >0 
   BEGIN

        SELECT @MaxSeq = ISNULL( Max(A.EnvSerl),0)
          FROM KPX_TCOMEnvItem AS A
          JOIN #TCOMEnv        AS B ON A.EnvSeq = B.EnvSeq
         WHERE A.CompanySeq  = @CompanySeq 
           AND B.WorkingTag = 'A'            
           AND B.Status = 0 
        
        UPDATE #TCOMEnv                
           SET EnvSerl  = @MaxSeq + DataSeq   
         WHERE WorkingTag = 'A'            
           AND Status = 0 
...
--exec KPX_SCMEnvCheck @xmlDocument=N'<ROOT>
--  <DataBlock3>
