...
IF OBJECT_ID('KPX_SEIS_PL_MOD_PLANQuery') IS NOT NULL   
    DROP PROC KPX_SEIS_PL_MOD_PLANQuery  
GO  
  
-- v2014.11.24  
  
-- (경영정보)손익 수정 계획-조회 by 이재천   
CREATE PROC KPX_SEIS_PL_MOD_PLANQuery  
...
AS   
    ...
    SELECT FSItemSeq AS AccSeq, 
           ...
           B.EstAmt 
      FROM #Temp AS A 
      JOIN KPX_TEIS_PL_MOD_PLAN AS B ON ( B.CompanySeq = @CompanySeq AND B.AccSeq = A.FSItemSeq AND B.PlanYM = @PlanYM AND B.BizUnit = @BizUnit ) 
     Order by A.RowNum 
    
    RETURN  
GO 
exec KPX_SEIS_PL_MOD_PLANQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
