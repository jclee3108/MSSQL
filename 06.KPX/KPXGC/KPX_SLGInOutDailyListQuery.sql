...
IF OBJECT_ID('KPX_SLGInOutDailyListQuery') IS NOT NULL 
    DROP PROC KPX_SLGInOutDailyListQuery
GO 

-- v2014.12.05 

-- 사이트테이블로 변경 및 WoNo 추가 by이재천
 ...
 *************************************************************************************************/
  -- 입출고조회 - 조회 
 CREATE PROC KPX_SLGInOutDailyListQuery
 ...
 AS
     ...
     SELECT A.InOutType, A.InOutSeq, -1, '0', 
       ...
       JOIN KPX_TPUMatOutEtcOutItem               AS E WITH(NOLOCK) ON ( A.CompanySeq = E.CompanySeq AND A.InOutType = E.InOutType AND A.InOutSeq = E.InOutSeq )
       LEFT OUTER JOIN KPX_TPUMatOutEtcOutItemSub AS F WITH(NOLOCK) ON ( E.CompanySeq = F.CompanySeq AND E.InOutType = F.InOutType AND E.InOutSeq = F.InOutSeq AND E.InOutSerl = F.InOutSerl AND F.InOutKind IN (8023008, 8023012) )
      ...
      EXEC _SCOMProgStatus @CompanySeq, 'KPX_TPUMatOutEtcOutItem', 1036004, '#Temp_InOutProg', 'InOutSeq', '', 'InOutType', '', '', '', '', '', 'CompleteCHECK', 1, 'Qty', 'STDQty', '', '', 'InOutSeq', 'InOutSerl', 'InOutType', 'KPX_TPUMatOutEtcOut', @PgmSeq
     ...
     EXEC _SCOMSourceTracking @CompanySeq, 'KPX_TPUMatOutEtcOutItem', '#Temp_InOutProg', 'InOutSeq', '', ''  
     ...
     SELECT D.MinorSeq, A.InOutType, A.InOutSeq, 8918001, '0', '0'
       FROM #Temp_InOutProg            AS X
            JOIN KPX_TPUMatOutEtcOut        AS A WITH(NOLOCK) ON X.InOutType  = A.InOutType
            ...
            JOIN ( SELECT E.CompanySeq, E.InOutType, E.InOutSeq, MAX(E.PJTSeq)  AS PJTSeq, 
                     ...
                     FROM KPX_TPUMatOutEtcOutItem           AS E WITH(NOLOCK)
     ...
     UPDATE #JumpPgmId
        SET IsPMS = '1'
       FROM #JumpPgmId                AS A
            JOIN KPX_TPUMatOutEtcOut       AS B WITH(NOLOCK) ON A.InOutSeq   = B.InOutSeq
     ...
     UPDATE #JumpPgmId
       ...
       FROM #JumpPgmId              AS A
            JOIN KPX_TPUMatOutEtcOut     AS B WITH(NOLOCK) ON A.InOutSeq   = B.InOutSeq
                                                     AND A.InOutType  = B.InOutType
            JOIN KPX_TPUMatOutEtcOutItem AS C WITH(NOLOCK) ON B.InOutSeq   = C.InOutSeq  
     ...
     SELECT  A.InOutSeq      AS InOutSeq,
      ...
      FROM #Temp_InOutProg AS X
            ...
            JOIN ( SELECT E.CompanySeq, E.InOutType, E.InOutSeq,                                 -- 2012.06.25 윤보라 추가
                                     ...
                                     MAX(F.PJTName)     AS PJTName, 
                                     MAX(F.PJTNo)  AS PJTNo
                                FROM KPX_TPUMatOutEtcOutItem  AS E 
                                LEFT OUTER JOIN _TPJTProject AS F WITH (NOLOCK) ON E.PJTSeq     = F.PJTSeq 
                                                 AND E.CompanySeq = F.CompanySeq
                            GROUP BY E.CompanySeq, E.InOutType, E.InOutSeq ) AS G ON A.CompanySeq = G.CompanySeq 
                                                                                 AND A.InOutType  = G.InOutType  
           ...
           LEFT OUTER JOIN _TEQWorkOrderReqMasterCHE AS E WITH(NOLOCK) ON ( E.CompanySeq = @CompanySeq AND E.WOReqSeq = A.WOReqSeq ) 
