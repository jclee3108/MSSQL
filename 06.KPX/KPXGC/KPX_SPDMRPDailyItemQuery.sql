...
IF OBJECT_ID('KPX_SPDMRPDailyItemQuery') IS NOT NULL   
    DROP PROC KPX_SPDMRPDailyItemQuery  
GO  
  
-- v2014.12.15  
  
-- 일별자재소요계산-조회 by 이재천   
CREATE PROC KPX_SPDMRPDailyItemQuery  
...
AS   
    ...
    SELECT DISTINCT STUFF(STUFF(MRPDate,5,0,'-'),8,0,'-'), MRPDate
      FROM #BaseData 
     ORDER BY MRPDate 
    ...
    SELECT ItemSeq, MRPDate, Value, KindSeq 
      FROM #BaseData 
...
exec KPX_SPDMRPDailyItemQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
