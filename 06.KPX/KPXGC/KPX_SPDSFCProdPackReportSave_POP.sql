...
AS       
    ...
    BEGIN       
        ...
        --------------------------------------------------------------------------------------------------------------------------------        
        -- 전함바 데이터 Lot대체, 이동처리 기본데이터 및 채번 (Temp Table)
        --------------------------------------------------------------------------------------------------------------------------------        
        ...
        UPDATE #LotReplace              
           SET LotInOutSeq = @Seq + IDX_NO         
          FROM #LotReplace       
         WHERE WorkingTag = 'A'       
              
        -- 삭제, 수정  일 경우 연결 연결 테이블 InOutSeq,Serl 로 업데이트       
        UPDATE A       
           SET LotInOutSeq = B.InOutSeq      
        ...
        END       
        --------------------------------------------------------------------------------------------------------------------------------        
        -- 전함바 데이터 Lot대체, 이동처리 기본데이터 및 채번 (Temp Table), END 
        --------------------------------------------------------------------------------------------------------------------------------        
        ...
        BEGIN         
            ...
            /*
            -- Seq, No 채번 ( 전함바 제외 한 생산수량 Lot 대체 )
            */
            SELECT @BizUnit = MAX(BizUnit),              
                   @Date    = MAX(InOutDate)        
              FROM #LotReplace2         
            ...
            END       
            /*
            -- Seq, No 채번 ( 전함바 제외 한 생산수량 이동처리 ( 일반창고 -> 제품창고 ) ) 
            */      
            SELECT @BizUnit = MAX(BizUnit),              
                   @Date    = MAX(InOutDate)        
              FROM #LotReplace2         
            ...
            */ 
            SELECT @XmlData = CONVERT(NVARCHAR(MAX), ( SELECT A.WorkingTag,         
                                                              ...
                                                              1 AS Selected,         
                                                              0 AS Status,         
                                                              A.LotInOutSeq AS InOutSeq,         
                                                              A.BizUnit,         
                                                              A.LotInOutNo AS InOutNo,         
                                                              0 AS DeptSeq,         
                                                              1 AS EmpSeq,         
                                                              A.InOutDate,         
            ...
            UPDATE B        
               SET B.ProcYn = '2', 
                   B.ErrorMessage = '전함바 LOT 대체처리 오류'    
              FROM #BaseData AS A         
              JOIN KPX_TPDSFCWorkReport_POP AS B WITH(NOLOCK) ON ( B.CompanySeq = @CompanySeq AND B.Seq = A.Seq )      
            ...
            */
            ...
            /*
            -- Seq, No 채번 ( 전함바 제외한 생산수량 이동처리 ( 일반창고 -> 함바창고 ) ) 
            */
            SELECT @BizUnit = MAX(BizUnit),              
                   @Date    = MAX(InOutDate)        
              FROM #LotReplace2         
            ...
            /*
            -- Seq, No 채번 ( 전함바 제외한 생산수량 이동처리 ( 일반창고 -> 드레인창고 ) ) 
            */
            SELECT @Count = (SELECT COUNT(1) FROM #LotReplace2)         
        ...
        --select *from _TLGInOutDailyItem where inoutseq = 1322909
        --select @Seq 
        --return 
        -- Temp Talbe 에 생성된 키값 UPDATE       
        UPDATE #SubTable              
