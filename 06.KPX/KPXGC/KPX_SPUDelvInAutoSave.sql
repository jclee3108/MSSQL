...
IF OBJECT_ID('KPX_SPUDelvInAutoSave') IS NOT NULL
    DROP PROC KPX_SPUDelvInAutoSave
GO 
...
-- KPX용 자동입고 by이재천 
/*************************************************************************************************
수입검사대상품목등록 화면에서 자동입고미사용 시 
자동입고에 제외 되는 로직 

커스텀 작업이기 때문에 기존 패키지 자동입고는 환경설정에서 사용안함을 기본으로 한다. 
패키지에서 자동입고가 되지 않고 커스텀작업으로 추가된 부분에서 제외된 품목만 
자동미입고 처리한다. 
*************************************************************************************************/
/************************************************************
 설  명 - 구매 자동입고 
 ...
 ************************************************************/
 CREATE PROC KPX_SPUDelvInAutoSave
 ...
 --            @DelvInDate   NCHAR(8),
    ...
    -- KPX 로직에서는 자동입고일 경우 해당 로직은 제외 된다. ( 패키지 환경설정 자동입고 체크 제외 ) 
    IF ISNULL(@QCAutoIn, '0') IN ('1','True') 
    BEGIN
        ...
        -- KPX 사이트 테이블로 변경 
          DELETE #TPUDelvIn
           FROM #TPUDelvIn                         AS A
                JOIN _TPUDelvItem                  AS B ON A.DelvSeq  = B.DelvSeq
                                                       AND A.DelvSerl = B.DelvSerl
                LEFT OUTER JOIN KPX_TQcInReceiptItem AS C ON B.CompanySeq = C.CompanySeq
                                                       ANd B.ItemSeq    = C.ItemSeq
 ...
 --            AND A.Status = 0  
  ...
  END     
    ...
    BEGIN 
        UPDATE #TPUDelvIn
           SET DelvInDate = B.DelvDate
          FROM #TPUDelvIn AS A
          CROSS APPLY (SELECT MAX(DelvDate) AS DelvDate 
                         FROM _TPUDelv AS Z 
