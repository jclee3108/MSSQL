...
IF OBJECT_ID('KPX_SSLReceiptPlanExpSave') IS NOT NULL 
    DROP PROC KPX_SSLReceiptPlanExpSave
GO 

-- v2014.12.19 
    
-- 채권수금계획(수출)-저장 by 이재천     
CREATE PROC KPX_SSLReceiptPlanExpSave    
    ...
    BEGIN   
        ...
        BEGIN  
            DELETE B     
              ...
              JOIN KPX_TLReceiptPlanDom AS B ON ( B.CompanySeq = @CompanySeq AND A.PlanYM = B.PlanYM AND B.PlanType = '2' )      
        ...
        BEGIN   
            DELETE B     
              FROM #KPX_TLReceiptPlanDom AS A     
              JOIN KPX_TLReceiptPlanDom AS B ON ( B.CompanySeq = @CompanySeq AND A.PlanYM = B.PlanYM AND A.Serl = B.Serl AND B.PlanType = '2' )      
             WHERE A.WorkingTag = 'D'     
               AND A.Status = 0    
        END  
