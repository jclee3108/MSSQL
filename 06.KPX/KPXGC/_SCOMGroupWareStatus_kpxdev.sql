...
IF OBJECT_ID('_SCOMGroupWareStatus_kpxdev') IS NOT NULL 
    DROP PROC _SCOMGroupWareStatus_kpxdev
GO 

-- v2015.01.29 

-- 휴가신청(경조사내역데이터생성) by이재천 
CREATE PROC _SCOMGroupWareStatus_kpxdev
    @pWorkingTag      NCHAR(1),  
    @CompanySeq       INT = 1,      
    @LanguageSeq      INT = 1,      
    @UserSeq          INT = 0,      
...
AS   
    ...
    IF @pWorkingTag = 'E' AND @WorkKind = 'EmpVac' -- 휴가신청 SP 처리 
    BEGIN 
        DECLARE @EmpSeq     INT, 
                @VacSeq     INT, 
                @Seq        INT
