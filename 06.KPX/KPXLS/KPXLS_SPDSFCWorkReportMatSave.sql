IF OBJECT_ID('KPXLS_SPDSFCWorkReportMatSave') IS NOT NULL 
    DROP PROC KPXLS_SPDSFCWorkReportMatSave
GO 
 ...
 ************************************************************/ 
 CREATE PROC dbo.KPXLS_SPDSFCWorkReportMatSave
 ...
 --    EXEC _SCOMInsertColumnList '_TPDSFCMatinput'    
     ...
     EXEC _SCOMLog  @CompanySeq,    
                    ...
                    'CompanySeq,WorkReportSeq,ItemSerl,InputDate,MatItemSeq,MatUnitSeq,Qty,StdUnitQty,RealLotNo,SerialNoFrom,ProcSeq,AssyYn,IsConsign,GoodItemSeq,InputType,IsPaid,IsPjt,PjtSeq,WBSSeq,LastUserSeq,LastDateTime,Remark,ProdWRSeq'    
   ...
   IF EXISTS (SELECT 1 FROM #TPDSFCMatinput WHERE WorkingTag = 'A' AND Status = 0)      
     BEGIN      
         INSERT INTO _TPDSFCMatinput     
             ...
             SELECT @CompanySeq        ,A.WorkReportSeq      ,A.ItemSerl           ,A.InputDate              ,A.MatItemSeq       ,    
                     A.MatUnitSeq      , A.Qty               , 0        , A.RealLotNo             , A.SerialNoFrom      ,     
                     A.ProcSeq         , A.AssyYn            , A.IsConsign         , A.GoodItemSeq       ,     
                     A.InputType       , A.IsPaid            , A.IsPjt             , ISNULL(A.PjtSeq,0)                , A.WBSSeq            ,    
       @UserSeq           ,GETDATE()           , A.Remark            , A.ProdWRSeq    
               FROM #TPDSFCMatinput AS A       
 ...
 --            EXEC _SLGInOutMinusCheck @CompanySeq, '#TLGInOutDailyBatch'    
         ...
         INSERT INTO  _TPDMMOutItem    
             ...
             SELECT @CompanySeq      ,M.MatOutSeq        ,A.ItemSerl         ,A.MatItemSeq       ,    
                     ISNULL((SELECT OutWHSeq    
                               FROM _TDAItemStdWh     
                              WHERE CompanySeq = M.CompanySeq    
                                AND ItemSeq = A.MatItemSeq    
                                AND FactUnit = M.FactUnit    
                   AND OutWHSeq > 0),   M.OutWHSeq),                      -- 품목별 기본창고 아니면 워크센터의 출고창고.    
