...
IF OBJECT_ID('mnpt_SPJTOperatorPriceCheck') IS NOT NULL   
    DROP PROC mnpt_SPJTOperatorPriceCheck  
GO  
    
-- v2017.09.19
  
-- 운전원노임단가입력-SS1체크 by 이재천
CREATE PROC mnpt_SPJTOperatorPriceCheck      
    @ServiceSeq     INT = 0,         
...
AS       
    ...
    UPDATE #BIZ_OUT_DataBlock1  
      ...
      JOIN (SELECT S.StdDate  
              FROM (SELECT A1.StdDate  
                    ...
                    SELECT A1.StdDate  
                      FROM mnpt_TPJTOperatorPriceMaster AS A1  
    ...
    BEGIN  
        ...
        -- 키값생성코드부분 시작  
        EXEC @Seq = dbo._SCOMCreateSeq @CompanySeq, 'mnpt_TPJTOperatorPriceMaster', 'StdSeq', @Count  
