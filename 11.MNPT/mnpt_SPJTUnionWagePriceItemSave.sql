...
IF OBJECT_ID('mnpt_SPJTUnionWagePriceItemSave') IS NOT NULL   
    DROP PROC mnpt_SPJTUnionWagePriceItemSave  
GO  
    
-- v2017.09.28
  
-- 노조노임단가입력-SS2저장 by 이재천
CREATE PROC mnpt_SPJTUnionWagePriceItemSave
    @ServiceSeq     INT = 0,         
...
AS       
    ...
    EXEC _SCOMLog @CompanySeq   ,        
                  ...
                  '#DISTINCTData'    , -- 임시 테이블명        
                  'StdSeq,StdSerl'   , -- CompanySeq를 제외한 키( 키가 여러개일 경우는 , 로 연결 )        
                  @TableColumns , '', @PgmSeq  -- 테이블 모든 필드명   
    
    -- DELETE      
    IF EXISTS ( SELECT TOP 1 1 FROM #DISTINCTData WHERE WorkingTag = 'D' AND Status = 0 )    
    BEGIN    
          
        DELETE B   
          FROM #DISTINCTData                AS A   
          JOIN mnpt_TPJTUnionWagePriceItem  AS B ON ( B.CompanySeq = @CompanySeq AND A.StdSeq = B.StdSeq AND A.StdSerl = B.StdSerl )   
        ...
        SELECT A.WorkingTag, 
          ...
          INTO #ValueLog
          FROM #DISTINCTData                AS A 
          JOIN mnpt_TPJTUnionWagePriceValue AS B ON ( B.CompanySeq = @CompanySeq AND B.StdSeq = A.StdSeq AND B.StdSerl = A.StdSerl )
         WHERE A.Status = 0 
           AND A.WorkingTag = 'D' 
    ...
    -- UPDATE      
    IF EXISTS ( SELECT TOP 1 1 FROM #DISTINCTData WHERE WorkingTag = 'U' AND Status = 0 )    
    BEGIN  
          
        UPDATE B   
           SET B.PJTTypeSeq   = A.PJTTypeSeq,  
               B.UMLoadWaySeq = A.UMLoadWaySeq, 
               B.LastUserSeq  = @UserSeq    ,  
               B.LastDateTime = GETDATE()   ,
               B.PgmSeq       = @PgmSeq   
          FROM #DISTINCTData                AS A   
          JOIN mnpt_TPJTUnionWagePriceItem  AS B ON ( B.CompanySeq = @CompanySeq AND B.StdSeq = A.StdSeq AND B.StdSerl = A.StdSerl )
    ...
    -- INSERT  
    IF EXISTS ( SELECT TOP 1 1 FROM #DISTINCTData WHERE WorkingTag = 'A' AND Status = 0 )    
    BEGIN    
        ...
        SELECT @Companyseq, StdSeq, StdSerl, PJTTypeSeq, UMLoadWaySeq, 
