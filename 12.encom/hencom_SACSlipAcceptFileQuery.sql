IF OBJECT_ID('hencom_SACSlipAcceptFileQuery') IS NOT NULL 
    DROP PROC hencom_SACSlipAcceptFileQuery
GO 
...
작성자 - 김일주
************************************************************/
CREATE PROC hencom_SACSlipAcceptFileQuery
...
--  IF @PgmID IN ('FrmACSlipApprovalCash', 'FrmACSlipApproval', 'FrmACSlipApprovalAccount')
    ...
    -- select @PgmID
            DELETE #temp_TACSlipMaster 
             ...
             WHERE ((@PgmID = 'FrmACSlipApprovalCash' AND B.SMSetType IN (4148002, 0))
                    ...
                    OR  (@PgmID = 'FrmACSlipApproval' AND B.SMSetType IN (4148001, 4148002))  )
        END
    END 
    ELSE
    BEGIN
...
---------------------------------------- 
    ...
    -- 계정과목 조회조건 
    SELECT DISTINCT A.SlipMstSeq 
      INTO #AccSlipMstSeq
      FROM #temp_TACSlipMaster  AS A 
      JOIN _TACSlipRow          AS B WITH(NOLOCK) ON ( B.CompanySeq = @CompanySeq AND B.SlipMstSeq = A.SlipMstSeq ) 
     WHERE (@AccSeq = 0 OR B.AccSeq = @AccSeq) 

    
    -- 첨부파일여부 조회조건
    DECLARE @IsFile NCHAR(1) 

    SELECT @IsFile = ( 
                       CASE WHEN @IsFileSeq = 0 THEN '2' 
                            WHEN @IsFileSeq = 1 THEN '1' 
                            WHEN @IsFileSeq = 2 THEN '0' 
                            END 
                     ) 
    --select @IsFile 
    --return 
...
---------------------------------------- 
    SELECT A.SlipMstID, A.CheckValue, A.AptRemark, A.AptDate, 
           ...
           CASE WHEN EXISTS (SELECT 1 FROM hncomcommon.._TCAAttachFileData WHERE AttachFileSeq = Q.FileSeq) THEN '1' ELSE '0' END AS IsFile
      FROM #temp_TACSlipMaster AS A LEFT OUTER JOIN #SlipJumpPgmID AS B 
            ...
            --                                  AND D.SetSlipNo     = C.SetSlipNo
                                    ...
                                    LEFT OUTER JOIN hencom_TACSlipApprovalFile AS Q ON ( Q.CompanySeq = @CompanySeq AND Q.SlipMstSeq = A.SlipMstSeq ) 
                                               JOIN #AccSlipMstSeq AS QQ ON ( QQ.SlipMstSeq = A.SlipMstSeq ) 
     ...
     WHERE ((@DataIsSet = '1' AND A.IsSet = '1') OR  (ISNULL(@DataIsSet, '') <> '1' AND A.IsSet IN('', '0', '1', '2'))) 
       ...
       AND (@IsFile = '2' OR @IsFile = (CASE WHEN ISNULL(Q.FileSeq,0) = 0 THEN '0' ELSE '1' END))
...
Go
begin tran 
exec hencom_SACSlipAcceptFileQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
    ...
    <AccSeq />
    <AccUnit />
    <AccDateFrom>20170801</AccDateFrom>
    <SlipNoFrom />
    <AccDateTo>20170830</AccDateTo>
    <SlipNoTo />
    <RegEmpSeq>776</RegEmpSeq>
    <SlipUnit />
    <SetAccDateFrom />
    <SlipSetNoFrom />
    <SetAccDateTo />
    ...
    <AptEmpSeq />
    <SetEmpSeq />
    <IncludeSubDept>0</IncludeSubDept>
    <IsDoneGW>0</IsDoneGW>
  </DataBlock1>
</ROOT>',@xmlFlags=2,@ServiceSeq=1511893,@WorkingTag=N'',@CompanySeq=1,@LanguageSeq=1,@UserSeq=1,@PgmSeq=1029551
rollback 