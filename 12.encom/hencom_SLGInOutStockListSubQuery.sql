IF OBJECT_ID('hencom_SLGInOutStockListSubQuery') IS NOT NULL 
    DROP PROC hencom_SLGInOutStockListSubQuery
GO 
 ...
 ************************************************************/    
 ...
 CREATE PROC dbo.hencom_SLGInOutStockListSubQuery                    
...
--기타입고  
     ...
     SELECT  A.ItemSeq,    
             ...
             T.DeptName,    
             D.InOutDate,    
             CONVERT(DECIMAL(19,5),D.InQty) AS InQty,    
             CONVERT(DECIMAL(19,5),D.OutQty) AS OutQty,    
             ...
             CONVERT(DECIMAL(19,0),D.InAmt) AS InAmt,    
             CONVERT(DECIMAL(19,0),D.OutAmt) AS OutAmt,    
             CONVERT(DECIMAL(19,5),D.EtcInQty) AS EtcInQty,    
             CONVERT(DECIMAL(19,5),D.EtcOutQty) AS EtcOutQty,    
             CONVERT(DECIMAL(19,0),D.EtcInAmt) AS EtcInAmt,    
             CONVERT(DECIMAL(19,0),D.EtcOutAmt) AS EtcOutAmt,    
--             ----------
     ...
     JOIN(SELECT InOutDate,    
                 ...
                 FROM #TMPInOutData    
                 GROuP BY InOutDate,ItemSeq) AS D ON D.ItemSeq = A.ItemSeq    
    WHERE D.InOutDate BETWEEN @DateFr AND @DateTo    
      AND I.DeptSeq = @DeptSeq 
      AND A.ItemSeq = @ItemSeq 
    ...
    SELECT 
           ...
           SUM(ISNULL(A.StockQty,0)) AS StockQty, --실재고수량    
 ...
 RETURN
 GO
exec hencom_SLGInOutStockListSubQuery @xmlDocument=N'<ROOT>
  <DataBlock1>
    <WorkingTag />
    <IDX_NO>2</IDX_NO>
    <DataSeq>1</DataSeq>
    <Status>0</Status>
    <Selected>1</Selected>
    <ItemSeq>21247</ItemSeq>
    <DeptSeq>27</DeptSeq>
    <TABLE_NAME>DataBlock1</TABLE_NAME>
    <DateFr>20161201</DateFr>
    <DateTo>20161231</DateTo>
  </DataBlock1>
</ROOT>',@xmlFlags=2,@ServiceSeq=1037672,@WorkingTag=N'',@CompanySeq=1,@LanguageSeq=1,@UserSeq=1,@PgmSeq=1030796