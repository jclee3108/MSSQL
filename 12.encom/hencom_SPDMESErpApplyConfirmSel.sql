IF OBJECT_ID('hencom_SPDMESErpApplyConfirmSel') IS NOT NULL 
    DROP PROC hencom_SPDMESErpApplyConfirmSel
GO 
 ...
 ************************************************************/      
 CREATE PROC dbo.hencom_SPDMESErpApplyConfirmSel      
 ...
 AS         
  ...
  IF @@ERROR <> 0 RETURN        
     ...
     JOIN #hencom_TIFProdWorkReportCloseSum AS B ON B.SumMesKey = A.SumMesKey
...
--         AND ISNULL(MI.Qty,0) <> 0           --투입될 수량    
 ...
 GO
 BEGIN TRAN 
exec hencom_SPDMESErpApplyConfirmSel @xmlDocument=N'<ROOT>
  <DataBlock1>
    <WorkingTag />
    <IDX_NO>1</IDX_NO>
    <DataSeq>1</DataSeq>
