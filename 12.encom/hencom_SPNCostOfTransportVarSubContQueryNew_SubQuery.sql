...
IF OBJECT_ID('hencom_SPNCostOfTransportVarSubContNew_SubQuery') IS NOT NULL   
    DROP PROC hencom_SPNCostOfTransportVarSubContNew_SubQuery  
GO  
  
-- v2017.04.18 
  
-- 사업계획운송비변수등록_hencom-도급비조회 by 이재천
CREATE PROC hencom_SPNCostOfTransportVarSubContNew_SubQuery  
    @CompanySeq INT, 
    @DeptSeq    INT, 
    @PlanSeq    INT 
AS  
    ...
    INSERT INTO #Result (Gubun,ColumnA,ColumnB,ColumnC)        
    SELECT 1,'도급비','운송비','자차'        
    UNION         
    SELECT 2,'도급비','운송비','용차'        
    UNION         
    ...
    SELECT A.PJTRegSeq ,    
           ...
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '01' THEN ISNULL(D.SelfPrice01,0) ELSE 0 END  ) AS SelfPrice01,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '02' THEN ISNULL(D.SelfPrice02,0) ELSE 0 END  ) AS SelfPrice02,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '03' THEN ISNULL(D.SelfPrice03,0) ELSE 0 END  ) AS SelfPrice03,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '04' THEN ISNULL(D.SelfPrice04,0) ELSE 0 END  ) AS SelfPrice04,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '05' THEN ISNULL(D.SelfPrice05,0) ELSE 0 END  ) AS SelfPrice05,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '06' THEN ISNULL(D.SelfPrice06,0) ELSE 0 END  ) AS SelfPrice06,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '07' THEN ISNULL(D.SelfPrice07,0) ELSE 0 END  ) AS SelfPrice07,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '08' THEN ISNULL(D.SelfPrice08,0) ELSE 0 END  ) AS SelfPrice08,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '09' THEN ISNULL(D.SelfPrice09,0) ELSE 0 END  ) AS SelfPrice09,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '10' THEN ISNULL(D.SelfPrice10,0) ELSE 0 END  ) AS SelfPrice10,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '11' THEN ISNULL(D.SelfPrice11,0) ELSE 0 END  ) AS SelfPrice11,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '12' THEN ISNULL(D.SelfPrice12,0) ELSE 0 END  ) AS SelfPrice12, 

           MAX( CASE WHEN RIGHT(B.BPYm,2) = '01' THEN ISNULL(E.LentPrice01,0) ELSE 0 END  ) AS LentPrice01,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '02' THEN ISNULL(E.LentPrice02,0) ELSE 0 END  ) AS LentPrice02,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '03' THEN ISNULL(E.LentPrice03,0) ELSE 0 END  ) AS LentPrice03,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '04' THEN ISNULL(E.LentPrice04,0) ELSE 0 END  ) AS LentPrice04,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '05' THEN ISNULL(E.LentPrice05,0) ELSE 0 END  ) AS LentPrice05,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '06' THEN ISNULL(E.LentPrice06,0) ELSE 0 END  ) AS LentPrice06,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '07' THEN ISNULL(E.LentPrice07,0) ELSE 0 END  ) AS LentPrice07,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '08' THEN ISNULL(E.LentPrice08,0) ELSE 0 END  ) AS LentPrice08,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '09' THEN ISNULL(E.LentPrice09,0) ELSE 0 END  ) AS LentPrice09,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '10' THEN ISNULL(E.LentPrice10,0) ELSE 0 END  ) AS LentPrice10,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '11' THEN ISNULL(E.LentPrice11,0) ELSE 0 END  ) AS LentPrice11,    
           MAX( CASE WHEN RIGHT(B.BPYm,2) = '12' THEN ISNULL(E.LentPrice12,0) ELSE 0 END  ) AS LentPrice12
      INTO #PlanBaseData 
      ...
      JOIN #SelfPrice                   AS D ON ( D.UMDistanceDegree = C.UMDistanceDegree ) 
      JOIN #LentPrice                   AS E ON ( 1 = 1 ) 
     WHERE A.CompanySeq = @CompanySeq 
       AND A.PlanSeq = @PlanSeq 
       AND A.DeptSeq = @DeptSeq 
       AND ISNULL(A.ItemSeq,0) <> 0
     GROUP BY A.PJTRegSeq, B.BPYm 
      ...
      JOIN (
            SELECT Z.PlanSeq, 
              ...
              WHERE Z.CompanySeq = @CompanySeq  
                ...
                AND Z.DeptSeq = @DeptSeq  
           ) AS B ON ( B.BPYm = A.BPYm ) 
     WHERE A.Qty <> 0 
    
    -- 월별 금액 집계 
    SELECT RIGHT(BPYm,2) AS YM, SUM(SelfAmt) AS SelfAmt, SUM(LentAmt) AS LentAmt
      INTO #SumCarAmt 
      FROM #CarAmt 
     GROUP BY BPYm
    
    -- 세로 데이터 가로로 변경 
    SELECT B.[01] AS SelfAmt01, B.[02] AS SelfAmt02, B.[03] AS SelfAmt03, B.[04] AS SelfAmt04, B.[05] AS SelfAmt05, B.[06] AS SelfAmt06, 
    ...
    SELECT B.[01] AS LentAmt01, B.[02] AS LentAmt02, B.[03] AS LentAmt03, B.[04] AS LentAmt04, B.[05] AS LentAmt05, B.[06] AS LentAmt06, 
    ...
    UPDATE A
      ...
      FROM #Result AS A 
      LEFT OUTER JOIN #SelfAmt AS B ON ( 1 = 1 ) 
      LEFT OUTER JOIN #LentAmt AS C ON ( 1 = 1 ) 
    ...
    SELECT A.PJTRegSeq, 
      ...
      INTO #OilAmt
      FROM #QtyPrice    AS A 
      JOIN #TotalKm     AS B ON ( B.BPYm = A.BPYm ) 
      JOIN (
            SELECT Z.PlanSeq, 
                   Z.BpYm, 
                   (100 - Z.LentUseRate) / 100 AS SelfUseRate, 
              ...
              WHERE Z.CompanySeq = @CompanySeq  
                AND Z.PlanSeq = @PlanSeq 
                AND Z.DeptSeq = @DeptSeq  
           ) AS C ON ( C.BPYm = A.BPYm ) 
      JOIN #OilPirce AS D ON ( D.BPYm = A.BPYm ) 
     WHERE A.Qty <> 0 

    -- 월별 금액 집계 
    SELECT RIGHT(BPYm,2) AS YM, SUM(SelfOilAmt) AS SelfAmt, SUM(LentOilAmt) AS LentAmt
      INTO #SumOilAmt 
      FROM #OilAmt 
     GROUP BY BPYm
    
    -- 세로 데이터 가로로 변경 
    SELECT B.[01] AS SelfOilAmt01, B.[02] AS SelfOilAmt02, B.[03] AS SelfOilAmt03, B.[04] AS SelfOilAmt04, B.[05] AS SelfOilAmt05, B.[06] AS SelfOilAmt06, 
    ...
    SELECT B.[01] AS LentOilAmt01, B.[02] AS LentOilAmt02, B.[03] AS LentOilAmt03, B.[04] AS LentOilAmt04, B.[05] AS LentOilAmt05, B.[06] AS LentOilAmt06, 
    ...
    UPDATE A
      ...
      FROM #Result AS A 
      LEFT OUTER JOIN #SelfOilAmt AS B ON ( 1 = 1 ) 
      LEFT OUTER JOIN #LentOilAmt AS C ON ( 1 = 1 ) 
    ...
    SELECT A.PJTRegSeq, 
      ...
      INTO #OTAmt
      FROM #QtyPrice        AS A 
      JOIN #AddTimePrice    AS B ON ( B.BPYm = A.BPYm ) 
      JOIN (
            SELECT Z.PlanSeq, 
                   Z.BpYm, 
    ...
    -- 월별 금액 집계 
    SELECT RIGHT(BPYm,2) AS YM, SUM(OTAmt) AS OTAmt 
      INTO #SumOTAmt 
      FROM #OTAmt 
     GROUP BY BPYm
    

    SELECT B.[01] AS OTAmt01, B.[02] AS OTAmt02, B.[03] AS OTAmt03, B.[04] AS OTAmt04, B.[05] AS OTAmt05, B.[06] AS OTAmt06, 
    ...
    UPDATE A
       SET Mth1    = ISNULL(B.OTAmt01,0),  
           ...
           Total   = ISNULL(B.OTAmt01,0) +
                     ...
                     ISNULL(B.OTAmt12,0)
      FROM #Result AS A 
      LEFT OUTER JOIN #OT AS B ON ( 1 = 1 ) 
     WHERE A.Gubun = 5 
    ...
    SELECT A.PJTRegSeq, 
      ...
      INTO #ReturnAmt
      FROM #QtyPrice        AS A 
      JOIN #ReturnPrice    AS B ON ( B.BPYm = A.BPYm ) 
      JOIN (
            SELECT Z.PlanSeq, 
                   Z.BpYm, 
    ...
    -- 월별 금액 집계 
    SELECT RIGHT(BPYm,2) AS YM, SUM(ReturnAmt) AS ReturnAmt 
      INTO #SumReturnAmt 
      FROM #ReturnAmt 
     GROUP BY BPYm

    SELECT B.[01] AS ReturnAmt01, B.[02] AS ReturnAmt02, B.[03] AS ReturnAmt03, B.[04] AS ReturnAmt04, B.[05] AS ReturnAmt05, B.[06] AS ReturnAmt06, 
    ...
    UPDATE A
      ...
      FROM #Result AS A 
      LEFT OUTER JOIN #Return AS B ON ( 1 = 1 ) 
     WHERE A.Gubun = 6 
    ----------------------------------------------------------------------------
    -- 회수수 운반비,END
    ----------------------------------------------------------------------------
    ...
    -- 식대, 기타(산재,기타,소계)
    ----------------------------------------------------------------------------
    SELECT RIGHT(A.BPYm,2) AS YM, SUM(A.PayforMeal) AS PayforMeal, SUM(A.IndusAccidInsur) AS IndusAccidInsur, SUM(A.EtcCost) AS EtcCost
      INTO #SumEtc 
      FROM hencom_TPNCostOfTransportVar AS A 
     WHERE A.CompanySeq = @CompanySeq 
       AND A.DeptSeq = @DeptSeq 
       AND A.PlanSeq = @PlanSeq 
     GROUP BY RIGHT(A.BPYm,2) 

    SELECT B.[01] AS PayforMeal01, B.[02] AS PayforMeal02, B.[03] AS PayforMeal03, B.[04] AS PayforMeal04, B.[05] AS PayforMeal05, B.[06] AS PayforMeal06, 
    ...
    SELECT B.[01] AS IndusAccidInsur01, B.[02] AS IndusAccidInsur02, B.[03] AS IndusAccidInsur03, B.[04] AS IndusAccidInsur04, B.[05] AS IndusAccidInsur05, B.[06] AS IndusAccidInsur06, 
    ...
    SELECT B.[01] AS EtcCost01, B.[02] AS EtcCost02, B.[03] AS EtcCost03, B.[04] AS EtcCost04, B.[05] AS EtcCost05, B.[06] AS EtcCost06, 
       ...
       SET Mth1    = CASE WHEN A.Gubun = 7 THEN ISNULL(B.PayforMeal01,0) 
           ...
           Total   = CASE WHEN A.Gubun = 7 THEN ISNULL(B.PayforMeal01,0) 
                     ...
                     CASE WHEN A.Gubun = 7 THEN ISNULL(B.PayforMeal12,0) 
                          ...
                          END   
      FROM #Result AS A 
      LEFT OUTER JOIN #PayforMeal       AS B ON ( 1 = 1 ) 
      LEFT OUTER JOIN #IndusAccidInsur  AS C ON ( 1 = 1 ) 
      LEFT OUTER JOIN #EtcCost          AS D ON ( 1 = 1 ) 
    ...
    -- 도급단가
    ----------------------------------------------------------------------------
    SELECT RIGHT(A.BPYm,2) AS YM, SUM(A.Qty) AS Qty 
      INTO #SumQty
      FROM #QtyPrice AS A 
     GROUP BY RIGHT(A.BPYm,2)
    ...
    -- 세로 데이터 가로로 변경 
    SELECT B.[01] AS Qty01, B.[02] AS Qty02, B.[03] AS Qty03, B.[04] AS Qty04, B.[05] AS Qty05, B.[06] AS Qty06, 
    ...
    -- 월별 용차사용율
    SELECT RIGHT(BPYm,2) AS YM, MAX(LentUseRate) / 100 AS LentUseRate 
      INTO #LentUseRate
      FROM hencom_TPNCostOfTransportVar 
     WHERE CompanySeq = @CompanySeq 
       AND DeptSeq = @DeptSeq 
       AND PlanSeq = @PlanSeq 
     GROUP BY RIGHT(BPYm,2)
    UNION ALL -- 용차 사용율 평균
    SELECT 13 AS YM, (SUM(LentUseRate)/12) / 100 AS LentUseRate 
      FROM hencom_TPNCostOfTransportVar 
     WHERE CompanySeq = @CompanySeq 
       AND DeptSeq = @DeptSeq 
       AND PlanSeq = @PlanSeq 
    ...
    --return 
    -- 세로 데이터 가로로 변경 
    SELECT B.[01] AS LentUseRate01, B.[02] AS LentUseRate02, B.[03] AS LentUseRate03, B.[04] AS LentUseRate04, B.[05] AS LentUseRate05, B.[06] AS LentUseRate06, 
    ...
    UPDATE A
      ...
      FROM #Result          AS A 
      JOIN #SelfTotalAmt    AS B ON ( 1 = 1 ) 
      JOIN #TotalQty        AS C ON ( 1 = 1 ) 
      JOIN #LentUseRate_Row AS D ON ( 1 = 1 ) 
      JOIN #TotalAmt        AS E ON ( 1 = 1 ) 
     WHERE A.Gubun = 13
    ----------------------------------------------------------------------------
    -- 자차단가달성율, END 
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    -- 운반비
    SELECT  ISNULL(B.ValueText,'0') AS IsCarGubun, --1 자차, 0 용차
     ...
     GROUP BY ISNULL(ValueText,'0')
    
    UPDATE A
       SET Sales = CASE WHEN A.Gubun = 1 THEN B.Amt ELSE C.Amt END 
      FROM #Result AS A 
      JOIN #TMP_hencom_TPUSubContrCalc AS B ON ( B.IsCarGubun = '1' ) 
      JOIN #TMP_hencom_TPUSubContrCalc AS C ON ( C.IsCarGubun = '0' ) 
     WHERE Gubun IN ( 1, 2 ) 
    
    --유류비
    SELECT ISNULL(C.ValueText,'0') AS IsCarGubun, --1 자차, 0 용차
    ...
    UPDATE A
       SET Sales = CASE WHEN A.Gubun = 3 THEN B.Amt ELSE C.Amt END 
      FROM #Result AS A 
      JOIN #TMP_hencom_TPUFuelCalc AS B ON ( B.IsCarGubun = '1' ) 
      JOIN #TMP_hencom_TPUFuelCalc AS C ON ( C.IsCarGubun = '0' ) 
     WHERE Gubun IN ( 3, 4 ) 
    ...
    UPDATE A
      ...
      FROM #Result AS A 
      JOIN #Result AS B ON ( B.Gubun = 11 ) 
      JOIN ( 
            SELECT SUM(ISNULL(Qty,0)) AS SalesQty
    ...
    ------------------
    -- 전체금액
    SELECT SUM(ISNULL(A.Sales,0)) AS Amt
    ...
    -- 실적 자차사용율
    SELECT SUM(CASE WHEN ISNULL(D.IsSelf,'0') = '1' THEN A.Qty ELSE 0 END) AS SelfQty, 
    ...
    UPDATE A
      ...
      FROM #Result              AS A 
      JOIN #SalesSelfTotalAmt   AS B ON ( 1 = 1 ) 
      JOIN #SalesTotalQty       AS C ON ( 1 = 1 ) 
      JOIN #SalesSelfUseRate    AS D ON ( 1 = 1 ) 
      JOIN #SalesTotalAmt       AS E ON ( 1 = 1 ) 
     WHERE A.Gubun = 13
    ----------------------------------------------------------------------------
    -- 실적, END 
    ----------------------------------------------------------------------------
...
--EXEC hencom_SPNCostOfTransportVarSubContNew_SubQuery 1, 44, 3
