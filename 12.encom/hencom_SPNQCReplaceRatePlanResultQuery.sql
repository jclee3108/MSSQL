IF OBJECT_ID('hencom_SPNQCReplaceRatePlanResultQuery') IS NOT NULL 
    DROP PROC hencom_SPNQCReplaceRatePlanResultQuery
GO 
 ...
 ************************************************************/      
 ...
 CREATE PROC dbo.hencom_SPNQCReplaceRatePlanResultQuery                     
 ...
 -- 사업계획년도와 전년도에 해당하는 표준배합자재들을 담는다.     
  ...
  --  전년도 제품매출계획  
     ...
     AND A.ItemSeq <> 0  --제품인 경우.  
     --년도의 차수중에 확정된 차수를 대상으로 한다.  
 ...
 --자재매입단가: 소수점첫째자리에서 반올림처리.  
   ...
   --이전년도의 월별 자재매입단가의 평균단가
     ...
     WHERE A.PlanSeq = @PrevCfmPlanSeq  --같은 년도의 차수중에 확정된 차수.
   

  
     SELECT P.MatItemSeq                                 AS MatItemSeq, --투입자재  
...
--     select * from #TMPIFProdWorkReportCloseSumThis return
  --최종조회  
     ...
     SELECT A.ItemSeq,  
             ...
             MatPrev.Qty                             AS PrevQty , --전년도 실적  
             MatPrev.Amt                             AS PrevAmt, --전년도 실적  
             MatPrev.Amt / MatPrev.Qty               AS PrevPrice, --전년도 실적  
             MPPrev.MatItemSeq                       AS MatItemSeq,--전년도 사업계획 자재  
             MPPrev.MatInputQty                      AS PrevTotQty, --전년도 사업계획 투입자재 수량  
             MPPrev.MatInputAmt                      AS PrevTotAmt , --전년도 사업계획 투입자재 금액  
             MPPrev.MatInputAmt / MPPrev.MatInputQty AS PrevAvgPrice , --전년도 사업계획 투입자재 단가 
  ...
  SELECT  -1 AS ItemSeq,  
             ...
             0 AS ItemClassLSeq  ,  
             STP.OutQty                          AS PrevQty , --전년도 실적  
             STP.CurAmt                          AS PrevAmt, --전년도 실적  
             STP.CurAmt/STP.OutQty               AS PrevPrice, --전년도 실적  
             -1                                  AS MatItemSeq,--전년도 사업계획 자재  
             PrevTot.SalesQty                    AS PrevTotQty, --전년도 사업계획 투입자재 수량  
             PrevTot.SalesAmt                    AS PrevTotAmt , --전년도 사업계획 투입자재 금액  
             PrevTot.SalesAmt /PrevTot.SalesQty  AS PrevAvgPrice , --전년도 사업계획 투입자재 단가 
             S.Qty1 AS Qty1 ,
             S.Amt1 AS Amt1 ,
     ...
     LEFT OUTER JOIN ( select sum(Salesqty) as SalesQty, sum(SalesAmt) as SalesAmt   from #TMP_SalesPlanThis) AS ST ON 1=1  ---- 당해년도계획토탈    ---- 애는 다시만들어야 함
     LEFT OUTER JOIN #TMPIFProdWorkReportCloseSumThisTot AS STP ON STP.Gubun = A.Gubun AND STP.YY = @PrevYear -- 전년도실적토탈
